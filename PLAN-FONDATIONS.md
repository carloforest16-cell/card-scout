...
- [x] **4.2 — Skill verify-cardmetrics** — Fait · 5 juillet. `.claude/skills/verify-cardmetrics/SKILL.md` : lint+build+garde-fou import error, preview (port 3001, console, mobile 375px), fraîcheur des caches (`?refresh=1`, fire-and-forget), procédure complète pour vérifier une page authentifiée (test login + seed), smoke test, règle d'honnêteté sur les limites de vérification.
- [x] **4.3 — Skill loop-iteration** — Fait · 5 juillet. `.claude/skills/loop-iteration/SKILL.md` : protocole générique (sélection, lecture avant code, exécution complète, vérification, mise à jour du plan, commit, stop) applicable à tout futur `PLAN-*.md`, plus les garde-fous transversaux (jamais de vérification simulée, confirmation avant action sur système partagé même en mode "roule toutes les tâches").
- [x] **4.4 — Skill nouvelle-page** — Fait · 5 juillet. `.claude/skills/nouvelle-page/SKILL.md` : rappel du skill design en premier, tokens/primitives existants (couleurs, fonts, composants `cn-*`, patterns WOW génériques), règles dures (fr-CA, pas de fake data, pas d'emojis-icônes, touch targets, contraste, mobile), pièges CSS/animation connus.
- [x] **4.5 — Skill audit-code-mort** — Fait · 5 juillet. `.claude/skills/audit-code-mort/SKILL.md` : méthode en 5 étapes (grep sans filtre d'extension, remonter les importeurs réels, vérifier le JSX effectivement rendu, chercher les usages dynamiques, supprimer complètement une fois confirmé) + mention de `knip` (tâche 5.3) comme point de départ à vérifier manuellement, pas comme vérité absolue.
- [x] **4.6 — Skill toucher-au-score** — Fait · 5 juillet. `.claude/skills/toucher-au-score/SKILL.md` : architecture (`cardScoutScoreMath.js` = source unique des poids, volontairement sans `server-only` pour rester importable côté client ; `cardScoutScore.js` = orchestrateur serveur), les 13 facteurs et leurs poids exacts, la distinction fastMode vs sous-scores enrichis (cron `enrich-scores`), et la règle absolue : ne jamais modifier `SCORE_WEIGHTS` sans tâche produit explicite.
