...
- [x] **0.1 Write-through du recalcul live.** *(2026-07-05)* — `writeBackPlayerScore` ajouté dans `lib/playerScores.js`, appelé fire-and-forget dans `/api/score/route.js` après chaque recalcul live. Dans `/api/score` (`app/api/score/route.js`), quand le score stocké est périmé et qu'un recalcul live réussit, réécrire le résultat dans `player_scores` (réutiliser `upsertScoreRows` ou équivalent exporté de `lib/playerScores.js`). Résultat : la page joueur et le top opportunités lisent le même chiffre. Vérifier avec Carlsson : après visite de sa page, `SELECT score FROM player_scores WHERE player_name ILIKE '%carlsson%'` doit refléter le score affiché.
- [x] **0.2 Tie-break déterministe.** *(2026-07-05)* — `.order("points", desc).order("player_id", asc)` ajouté après score dans `opportunitesTop.js` et `playerScores.js`. Partout où on classe par score (`buildTopOpportunitesFromDb` dans `lib/opportunitesTop.js`, et tout `ORDER BY score` dans les API), ajouter un tie-break stable : `score DESC, points DESC, player_id ASC`. Un joueur à égalité ne doit plus disparaître/apparaître au hasard.
- [x] **0.3 Règle « DB d'abord » documentée.** *(2026-07-05)* — Section « Score Source of Truth » ajoutée dans CLAUDE.md avec règle write-through et tie-break. Ajouter dans `CLAUDE.md` (section Architecture) : `player_scores` est LA source de vérité des scores ; tout recalcul live doit écrire en retour (write-through) ; interdiction d'afficher un score calculé qui n'est pas persisté.

## Phase 1 — Annuaire complet : table `players`

- [x] **1.1 Migration Supabase.** *(2026-07-05)* — Table `players` créée avec index + RLS `service_role_full_access` + policy `public_read` pour anon. Table `players` : `player_id text PK`, `full_name text`, `first_name text`, `last_name text`, `team_abbrev text`, `position_code text`, `birth_date date`, `age int` (calculé au sync), `games_played int`, `goals int`, `assists int`, `points int`, `headshot_url text`, `sport text default 'NHL'`, `season_id int`, `is_active boolean default true`, `synced_at timestamptz`. Index sur `team_abbrev`, `position_code`, `points DESC`, et index trigram ou `lower(full_name)` pour la recherche. **Policy RLS `service_role_full_access` dans la MÊME migration** + policy `SELECT` pour `anon` (l'annuaire est public en lecture).
- [x] **1.2 `lib/playerDirectory.js`.** *(2026-07-05)* — `syncAllPlayers()` : fusionne summary+bios NHL, calcule l'âge, upsert par batch 100 dans `players`. Fonction `syncAllPlayers()` : réutilise `fetchAllSummaryForSeason` + `fetchAllBiosForSeason` (exportées par `lib/opportunitesTop.js`), fusionne summary+bios par `playerId`, calcule l'âge, upsert par batch de 100 dans `players` (`onConflict: "player_id"`). Inclut TOUS les patineurs de la saison (pas de filtre points) ; gardiens inclus si présents dans les données, sinon patineurs seulement pour l'instant (noter le choix). Retourne `{ synced, errors }`. Logs préfixés `[playerDirectory]`, timeouts sur tous les fetch (déjà dans `fetchNhlStatsJson`).
- [x] **1.3 Cron `sync-players`.** *(2026-07-05)* — Route cron + vercel.json (lundi 3h UTC) + handler direct dans trigger admin + carte dans AGENT_REGISTRY. `app/api/cron/sync-players/route.js` (auth `CRON_SECRET`, `recordCronRun` succès+échec, `maxDuration 300`). Schedule hebdomadaire dans `vercel.json` (les crons Hobby sont limités — vérifier le quota ; sinon le déclencher depuis un cron existant ou en manuel). Ajouter le handler direct dans `DIRECT_HANDLERS` du trigger admin + la carte agent dans le panel admin.
- [x] **1.4 Backfill initial + sentinelle.** *(2026-07-05)* — `sync-players` dans `EXPECTED_INTERVAL_HOURS` (9j margin), sentinelle `players` dans `buildHealthReport` (warn si <500 ou >14j). Backfill déclenché via panel admin après deploy. Lancer le sync une fois (trigger admin en prod, ou script local `scripts/syncPlayers.mjs` réutilisant la lib). Vérifier `SELECT count(*) FROM players` ≥ 600. Ajouter dans `/api/health` une sentinelle : `players_count < 500` ou `synced_at > 14 jours` → verdict `warn`.

## Phase 2 — Scores pour tout le monde (stratégie deux vitesses)

- [x] **2.1 Score math pour le pool étendu.** *(2026-07-05)* — `PROCESS_LIMIT` 75→100, `MATH_MIN_GP=10`, `MATH_CONCURRENCY=20` ajoutés. `buildPlayerPool()` retourne le pool complet (non tronqué). `computeRowForPlayer` tag `scoreMode:"full"`. Nouveau `computeMathRowForPlayer` : landing + `computeFactorScores(payload, null, null, null)`, `scoreMode:"math"`. `recomputeAllScores` traite top 100 en full, pool étendu (GP≥10 hors top 100) en math-only. Retourne `{ processed, written, fullPool, mathPool }`.
- [x] **2.2 UI honnête sur les scores math-only.** *(2026-07-05)* — `ScoreMathBadge` ajouté dans `spatial-player-showcase.jsx`. Quand `scoreMode === "math"` : badge "Score de base · stats uniquement", verdict et narratif DeepSeek masqués. `getTopStoredScores` et `buildTopOpportunitesFromDb` filtrent `.or("data->>scoreMode.is.null,data->>scoreMode.eq.full")` pour exclure les math-only du top opportunités.
...
- [x] **3.1 Design system d'abord.** *(2026-07-05)* — Pattern Marketplace/Directory, tokens CM existants (`--ice`, `--void`, `--surface`, `cn-card`). Animation 200-300ms, touch targets ≥44px.
- [x] **3.2 API `GET /api/joueurs`.** *(2026-07-05)* — `app/api/joueurs/route.js` : cache mémoire 5 min, merge `players` + `player_scores` en JS, filtres `search`/`team`/`position`, sort `score|points|name|age`, order `asc|desc`, pagination `page`/`limit` (défaut 25, max 100). Scores math-only exclus. Scorés en tête, non-scorés en fin.
- [x] **3.3 Page `/joueurs`.** *(2026-07-05)* — Server component (`page.js`) + client (`JoueursClient.js`). Grille de cartes responsive (1 col mobile, auto-fill 300px desktop). Avatar, nom, équipe, position, âge, PJ/B/A/Pts, score coloré par tier (tiret honnête si absent). Recherche débouncée 280ms, filtres équipe (33 équipes NHL)/position, tri cliquable avec ordre inversible, pagination 7-max avec flèches. Touch targets ≥44px, `prefers-reduced-motion` respecté, aria-labels sur tous les contrôles.
- [x] **3.4 Navigation + SEO.** *(2026-07-05)* — `AppNav.js` : lien "Joueurs" ajouté aux NAV_ITEMS (8e lien desktop) + drawer mobile. `BottomNav.js` : "Joueurs NHL" ajouté dans sheet "Le marché" (4 tabs = dans limite ≤5). Metadata server-side dans `page.js`. Sitemap TODO (phase 4).
- [x] **3.5 Vérification preview complète.** *(2026-07-05)* — `/joueurs` 200 (compilé 1770ms), `/api/joueurs` 200 (ok:true). HTML contient `jr-page`, `Joueurs NHL`. Table `players` vide localement (backfill en prod via panel admin) → état honnête "Aucun joueur trouvé" affiché. Zéro erreur console/serveur. Vérification avec données réelles en prod après sync.
