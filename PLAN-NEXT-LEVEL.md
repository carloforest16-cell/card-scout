...
- [ ] **1.1 — Purge des données synthétiques.** Dans `DashboardClient.js` : (a) `WatchlistWidget` — supprimer le fallback seed (lignes ~120-126) ; un joueur sans snapshot affiche son score courant (depuis `player_scores` via l'API summary, l'ajouter au payload si absent) et "— en observation" au lieu d'un delta inventé ; (b) `MarketPulseWidget` — supprimer le tableau `fallback` hardcodé ; si `marketPulse` est null/vide, masquer le widget entièrement ; (c) `PortfolioWidget` — si le sparkline n'est pas réel (`sparklineNote !== "real"`), afficher une barre de progression "Historique : jour X/30" au lieu d'une courbe simulée ; (d) KPI "Picks IA : 10" → afficher le vrai count depuis `opps` + la vraie date de refresh (le payload top-10 a un `generatedAt`). Critère : plus aucun nombre affiché qui ne vienne pas d'une vraie source.
