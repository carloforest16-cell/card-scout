...
function detectCardWarnings(title) {
  ...
  if (/\b(Fan\s*Art|Fan\s*Made|Novelty|Homemade|Art\s+Card|Custom\s+Card|Not\s+Official)\b/i.test(t))
    warnings.push({ type: "fake", label: "Carte non officielle — fan art, novelty ou fabrication maison" });
  ...
  if (/\bJumbo\b/i.test(t))
    warnings.push({ type: "jumbo", label: "Format Jumbo — version surdimensionnée, pas la carte de collection standard" });
  if (/\b(Oversized|Oversize)\b/i.test(t))
    warnings.push({ type: "jumbo", label: "Format surdimensionné — pas la carte standard du marché" });
  return warnings;
}

function CardWarnings({ title }) {
  const warnings = detectCardWarnings(title);
  if (!warnings.length) return null;
  return (
    <div className="an-warnings">
      {warnings.map((w, i) => (
                ...
                <div className="cn-card">
                  ...
                  <div className="an-listing">
                    ...
                    <div className="an-listing__body">
                      ...
                      <CardWarnings title={d.listing.title} />
