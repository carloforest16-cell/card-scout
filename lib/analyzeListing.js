...
import { scoreListingsForInvestment } from "@/lib/dealInvestmentScore";
import {
  detectCardGroup,
...
import { getMarketValue } from "@/lib/marketValue";
...
async function resolvePlayer(query, title) {
  ...
  try {
    ...
    console.error("[analyzeListing] resolvePlayer fetch failed:", err?.message ?? err);
    return null;
  }
...
async function fetchComparables(query, excludeId) {
  ...
  try {
    ...
    console.error("[analyzeListing] fetchComparables fetch failed:", err?.message ?? err);
    return [];
  }
  if (!res.ok) return [];
  let data;
  ...
  for (const it of items) {
    ...
    // `shippingCad` distingue trois états, et cette distinction compte pour la
    // comparaison des alternatives plus bas :
    //   0    → port gratuit ANNONCÉ par le vendeur (info fiable)
    //   > 0  → port connu
    //   null → eBay n'annonce rien (port calculé selon la destination) = INCONNU
    // `listingPriceToCad` renvoie null pour une valeur <= 0, donc le port
    // gratuit doit être détecté avant l'appel sous peine d'être confondu avec
    // un port inconnu.
    let ship = null;
    ...
    const shipAmount = ship ?? 0;
    out.push({
      title: typeof it?.title === "string" ? it.title : "",
      priceCad: Math.round((base + shipAmount) * 100) / 100,
      basePriceCad: base,
      shippingCad: ship,
...
export async function analyzeEbayListing(urlOrId) {
  ...
  // 1) Score joueur (réutilise le moteur Card Metrics, caché).
  let playerLanding = null;
  let cardScout = null;
  if (player) {
    playerLanding = await fetchPlayerLanding(player.playerId);
    if (playerLanding) {
      const landing = playerLanding;
  ...
  // Comparaison à base IDENTIQUE. `item.priceCad` inclut toujours le port (API
  // item detail), alors qu'un comparable dont eBay n'annonce pas le port n'a
  // que son prix de base. Comparer les deux directement présentait une annonce
  // à 127 $ + port inconnu comme « moins chère » qu'une à 105,94 $ + 24,46 $ —
  // alors que son coût réel était supérieur.
  // Règle : port connu des deux côtés → on compare les totaux. Port inconnu
  // côté comparable → on compare les prix de base. Conservateur par choix : on
  // préfère rater une vraie alternative que d'en annoncer une fausse.
  const itemBaseCad = Math.round((item.priceCad - (item.shippingCad ?? 0)) * 100) / 100;
  const compBasis = (c) => (c.shippingCad != null ? c.priceCad : c.basePriceCad);
  const itemBasis = (c) => (c.shippingCad != null ? item.priceCad : itemBaseCad);

  alternatives = matched
    .filter(
      (c) =>
        compBasis(c) < itemBasis(c) &&
        !isPickLot(c.title) &&
        isDualCard(c.title) === itemIsDual &&
        variantTag(c.title) === itemVariant
    )
    .sort((a, b) => compBasis(a) - compBasis(b))
    .slice(0, 4);
  ...
  // 2.5) Cote via lib/marketValue.js — préfère les ventes réelles 130point
  // quand assez de comps existent (≥3, fenêtre 120j), sinon garde le
  // fallback "ebay_active" ci-dessus tel quel (comportement inchangé).
  // /analyse n'avait jamais tenté 130point avant cette tâche.
  try {
    // `player.name` est indispensable : sans lui, `detectPlayerLastName` échoue
    // sur les titres qui placent le joueur APRÈS le set (« Upper Deck 2025-26
    // Series 1 Ivan Demidov … Young Guns »), l'empreinte renvoie null et on
    // retombait sur `buildComparableQuery` — qui SUPPRIME le n° de carte et
    // interroge donc 130point sur une cohorte plus large. D'où deux cotes
    // différentes pour la même carte entre /deals et /analyse.
    // `lib/dealFinder.js` passe déjà le nom de la même façon : une seule
    // construction de requête = une seule cote.
    const fp = extractCardFingerprint(item.title, player?.name);
    const soldQuery = fp?.searchQuery || query;
    const marketValue = await getMarketValue({
      query: soldQuery,
      sampleTitle: item.title,
      // Repli élargi si la requête fingerprint donne < 3 comps : la requête
      // de recherche originale de l'utilisateur, plus large.
      fallbackQuery: soldQuery !== query ? query : null,
      askingValueCad: fairValue?.fairValueCad ?? null,
      askingSampleSize: fairValue?.comps ?? 0,
    });
    if (marketValue.source === "sold") {
      const fair = marketValue.valueCad;
      fairValue = {
        fairValueCad: fair,
        source: "sold",
        comps: marketValue.sampleSize,
        confidence: marketValue.confidence,
        deltaPct: Math.round(((item.priceCad - fair) / fair) * 100),
        trusted: true, // ventes réelles ≥3 comps : plus fiable que le seuil eBay actif
      };
    }
    if (fairValue) fairValue.marketValueMeta = marketValue;
  } catch (err) {
    // Fallback : on garde fairValue tel que calculé ci-dessus (ou null) —
    // mais jamais en silence (cf. bug prod juin, cache servi 2 semaines sans trace).
    console.error("[analyzeListing] getMarketValue failed, fallback ebay_active:", err?.message ?? err);
  }
  ...
  // 3.5) Score d'investissement pour cette annonce précise.
  let listingScore = null;
  if (player?.name) {
    const marketPriceCad = fairValue?.trusted ? fairValue.fairValueCad : item.priceCad;
    const percentOfMarket = fairValue?.trusted
      ? Math.round((item.priceCad / fairValue.fairValueCad) * 100)
      : 100;
    let playerCtx = null;
    if (playerLanding) {
      const sub = playerLanding.featuredStats?.regularSeason?.subSeason;
      const gp = sub?.gamesPlayed != null ? Number(sub.gamesPlayed) : null;
      const goals = sub?.goals != null ? Number(sub.goals) : null;
      const assists = sub?.assists != null ? Number(sub.assists) : null;
      const points = sub?.points != null ? Number(sub.points) : null;
      const ppg = gp && gp > 0 && points != null ? Math.round((points / gp) * 1000) / 1000 : null;
      const bd = playerLanding.birthDate ?? playerLanding.player?.birthDate ?? null;
      playerCtx = {
        cardMetricsScore: cardScout?.score ?? null,
        ageYears: ageYearsFromBirthDate(bd),
        position: playerLanding.position ? String(playerLanding.position).toUpperCase() : null,
        teamAbbrev: playerLanding.currentTeamAbbrev ?? playerLanding.lastTeamAbbrev ?? null,
        pointsPerGame: ppg,
        gamesPlayed: gp,
        goals,
        assists,
    ...
    const scoreResult = await scoreListingsForInvestment(
      player.name,
      [{ listingIndex: 0, title: item.title, price: item.priceCad, groupType: group ?? "Unknown", marketPrice: marketPriceCad, percentOfMarket }],
      playerCtx
    );
    if (scoreResult.ok && scoreResult.scores?.length > 0) {
      listingScore = scoreResult.scores[0];
    }
  }
