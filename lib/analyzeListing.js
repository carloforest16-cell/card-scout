...
export async function analyzeEbayListing(urlOrId) {
  ...
  const playerQuery = item.playerHint || null;
  ...
  let fairValue = null;
  let alternatives = [];
  const comps = await fetchComparables(buildComparableQuery(item.title), itemId);
  ...
  if (matched.length >= 2) {
    ...
    const deltaPct = Math.round(((item.priceCad - fair) / fair) * 100);
    fairValue = {
      fairValueCad: fair,
      comps: prices.length,
