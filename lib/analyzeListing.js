...
import { fetchSportsCardsProValue } from "@/lib/sportsCardsPro";
...
/** Sets/parallèles reconnus, pour construire une requête SportsCardsPro nette. */
const SCP_SETS = [
  "Young Guns Renewed", "Young Guns", "Canvas", "Clear Cut", "Ultimate",
  "Artifacts", "Trilogy", "Premier", "SPx", "Allure", "Black Diamond",
  "The Cup", "Outburst", "Exclusives", "Dazzlers", "Ice", "MVP",
];

/**
 * Requête SportsCardsPro : joueur + set + année (4 chiffres) + n° de carte.
 * Format épuré qui matche le catalogue (ex. « Cole Caufield Young Guns 2021 201 »).
 */
function buildScpQuery(title, playerName) {
  const t = String(title ?? "");
  const year = (t.match(/\b(20\d{2})\b/) || [])[1] ?? "";
  const num = (t.match(/#\s*([A-Za-z]*-?\d+)/) || [])[1] ?? "";
  let setKw = "";
  for (const s of SCP_SETS) {
    if (new RegExp(s.replace(/\s+/g, "\\s+"), "i").test(t)) {
      setKw = s;
      break;
    }
  }
  return [playerName, setKw, year, num].filter(Boolean).join(" ").trim();
}
...
export async function analyzeEbayListing(urlOrId) {
  ...
  // Dual / checklist / multi-player = produit fondamentalement différent (ex.
  // « Levshunov / Demidov #250 » ≠ Young Guns solo Demidov #251). On ne mélange pas.
  const isDualCard = (t) =>
    /\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?\s*[/&]\s*[A-Z][a-z]+/.test(String(t ?? "")) ||
    /\bchecklist\b|\bdual\b|\bduo\b/i.test(String(t ?? ""));
  const itemIsDual = isDualCard(item.title);
  alternatives = matched
    .filter((c) => c.priceCad < item.priceCad && isDualCard(c.title) === itemIsDual)
    .sort((a, b) => a.priceCad - b.priceCad)
    .slice(0, 4);

  // Cote : SportsCardsPro (prix de VENTES réelles) en priorité.
  const cardNumber = (item.title.match(/#\s*([A-Za-z]*-?\d+)/) || [])[1] ?? null;
  const scp = player
    ? await fetchSportsCardsProValue(buildScpQuery(item.title, player.name), {
        lastName: player.name.split(/\s+/).pop(),
        cardNumber,
        grade,
      })
    : null;
  ...
  if (scp?.value != null) {
    fairValue = {
      fairValueCad: scp.value,
      source: "sportscardspro",
      confidence: "sold",
      trusted: true,
      deltaPct: Math.round(((item.priceCad - scp.value) / scp.value) * 100),
      byGrade: scp.byGrade,
      sourceUrl: scp.sourceUrl,
    };
  } else if (matched.length >= 2) {
    // Fallback : médiane des annonces actives eBay (moins fiable).
