...
// Sous 15 $, une « enchère chaude » n'a pas d'intérêt : les frais de port
// pèsent autant que la carte (audit 2026-09-23 : Kucherov Allure à 1 $ vs cote
// 6 $ affichée « −76 % » en vitrine de l'accueil).
const MIN_FAIR_VALUE_CAD = 15;
// Sans offre, le prix affiché est la mise de départ fixée par le vendeur, pas
// un prix de marché : « −76 % » sur une enchère à 0 offre ne dit rien du prix
// final. Une offre au moins = le marché a validé ce prix.
const MIN_BIDS_FOR_DEAL = 1;
...
async function buildAuctionDealsForPlayer(player, token, marketplaceId) {
  ...
  for (const a of auctions) {
    ...
    // Pas de repli « groupe » (médiane de toute la famille, tous millésimes
    // confondus) : comparer une Allure de base à la médiane de TOUTES les
    // Allure du joueur fabriquait de faux « −76 % ». Même règle que le Deal
    // Finder (bug B2) : une cote élargie ne déclenche jamais un signal de deal.
    if (fairValueCad == null) continue;
    if ((a.bidCount ?? 0) < MIN_BIDS_FOR_DEAL) continue;
