...
async function fetchEbayAuctionsForPlayer(playerName, token, marketplaceId) {
  ...
  // Nom de famille pour valider l'attribution : eBay renvoie parfois des lots
  // multi-joueurs sur une recherche floue (ex. « Jack Hughes » → lot Noesen/
  // Trouba/Bratt). Si le nom du joueur n'est pas dans le titre, on rejette.
  ...
  for (const item of summaries) {
    ...
    if (lastName.length >= 3 && !mapped.title.toLowerCase().includes(lastName)) {
      continue;
    }
...
async function buildAuctionDealsForPlayer(player, token, marketplaceId) {
  ...
  let fairMap = computeFairValueByFingerprint(comps);
  try {
