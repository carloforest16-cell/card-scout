...
/**
 * Série Upper Deck : « Series 1 », « Series 2 » et « Extended » sont trois
 * produits MUTUELLEMENT EXCLUSIFS. La même carte n'existe pas dans deux d'entre
 * eux, et leurs prix n'ont aucun rapport.
 *
 * Pourquoi un détecteur séparé de `SET_NAMES` : celui-ci exige
 * `\bExtended\s+Series\b`, alors que les vendeurs écrivent le plus souvent
 * « Upper Deck Extended » tout court. Une « Extended Outburst 1st Round Draft »
 * (~130 $) se retrouvait donc sans discriminant face aux ventes d'une
 * « Series 1 Young Guns Outburst #205 » (~656 $) — d'où un faux « −81 % ».
 *
 * `extended` est testé en premier : c'est un produit à part entière, et un titre
 * « Extended Series 1 » désigne l'Extended, pas la Series 1.
 * @param {string} title
 * @returns {"series-1" | "series-2" | "extended" | null}
 */
function detectSeriesTag(title) {
  if (/\bextended\b/i.test(title)) return "extended";
  if (/\bseries\s*(?:1|one)\b|\bs1\b/i.test(title)) return "series-1";
  if (/\bseries\s*(?:2|two)\b|\bs2\b/i.test(title)) return "series-2";
  return null;
}
...
/**
 * Sous-variantes qui MODIFIENT une carte au sein d'une même ligne de produit
 * sans être un parallèle coloré ni un grade — la source du bug B1
 * (PLAN-HOTTEST-DEALS.md). Ex. « Young Guns Retro » et « Young Guns » régulière
 * sont deux cartes DIFFÉRENTES avec deux cotes différentes, mais detectCardType
 * s'arrête à « young-guns » (première règle) et « Retro » se perdait → cohortes
 * fusionnées → cote empruntée (Carlsson Retro affichée « −81 % »).
 *
 * DISJOINT de PARALLEL_TAGS à dessein : Outburst/Dazzlers/High Gloss/Exclusives/
 * Rainbow Foil/Clear Cut sont DÉJÀ séparés via detectParallelTag — ne pas les
 * dupliquer ici (double token = risque de re-split incohérent). On ne capte que
 * les modificateurs encore invisibles pour la cohortKey.
 */
const VARIANT_TAGS = [
  { label: "retro", re: /\bRetro\b/i },
  { label: "canvas", re: /\bCanvas\b/i },
  { label: "deluxe", re: /\bDeluxe\b/i },
  { label: "portrait", re: /\bPortraits?\b/i },
  { label: "jumbo", re: /\bJumbo\b/i },
];

/**
 * @param {string} title
 * @param {string | null} [cardType] - pour éviter de dupliquer le type
 *   (« UD Canvas » seul : cardType=canvas ET variant=canvas → on garde null).
 * @returns {string | null}
 */
function detectVariantTag(title, cardType = null) {
  for (const v of VARIANT_TAGS) {
    if (v.re.test(title)) {
      // Le variant ne double jamais le cardType : il ne sert qu'à distinguer
      // deux cartes qui partageraient sinon exactement le même cardType.
      return v.label === cardType ? null : v.label;
    }
  }
  return null;
}
...
function detectParallelColor(title) {
  ...
  const hasParallelContext =
    ...
    // Gammes dont les inserts se déclinent en couleurs (Planet Metal Gold,
    // Precious Metal Gems…) : sans ça, « Planet Metal Gold » partageait la
    // cote de la Planet Metal de base (audit 2026-09-23).
    /\b(Metal\s+Universe|Precious\s+Metal|PMG|Prizm|Refractor)\b/i.test(title) ||
...
export function extractCardFingerprint(title, knownLastName = null) {
  ...
  const variantTag = detectVariantTag(t, type.cardType);
  const modifier = detectSearchModifier(t);

  // Fingerprint : on inclut toutes les dimensions qui rendent une carte UNIQUE.
  // Ordre : nom-année-type-variante-set-insert-parallèle-couleur-grade-code-tirage.
  ...
  // cohortKey : même chose SANS cardCode. Les vendeurs eBay incluent ou
  // omettent le #carte de façon aléatoire — un Demidov YG #205 et un
  // "Demidov YG" sans numéro sont la MÊME carte mais finissaient dans des
  // cohortes séparées, produisant des cotes différentes pour la même carte.
  const cohortParts = [
    lastName,
    year,
    type.cardType,
  ...
  const cohortKey = cohortParts.join("-");
...
export function extractPartialFeatures(title) {
  ...
  return {
    grade: grade?.grade ?? null,
    // `cardType` distingue les familles de cartes qui n'ont RIEN à voir entre
    // elles malgré un parallèle homonyme. Cas réel : une « Series 1 Young Guns
    // Outburst #205 » (vendue ~656 $) et une « Extended Outburst 1st Round
    // Draft » (vendue ~130 $) partagent parallelTag « outburst » et n'avaient
    // aucun autre trait distinctif — la première polluait la cote de la
    // seconde, produisant un faux « −81 % sous la cote ».
    cardType: type?.cardType ?? null,
    // Série Upper Deck (Series 1 / Series 2 / Extended) — produits mutuellement
    // exclusifs. Même règle que `cardType` : un null ne tranche jamais.
    seriesTag: detectSeriesTag(t),
    ...
    variantTag: detectVariantTag(t, type?.cardType ?? null),
  };
}
