...
/**
 * Prix vendus en lot par IDs de joueurs.
 * @param {string[]} playerIds
 * @returns {Promise<Map<string, number>>} playerId → ungradedCad
 */
export async function getStoredCardPricesByIds(playerIds) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from(TABLE)
      .select("player_id, ungraded_cad")
      .in("player_id", ids)
      .not("ungraded_cad", "is", null);
    if (error || !Array.isArray(data)) return new Map();
    const map = new Map();
    for (const row of data) {
      const v = Number(row.ungraded_cad);
      if (Number.isFinite(v) && v > 0) map.set(String(row.player_id), v);
    }
    return map;
  } catch {
    return new Map();
...
/**
 * Prix vendu par nom de joueur (recherche insensible à la casse).
 * @param {string} playerName
 * @returns {Promise<{ ungradedCad: number | null; productUrl: string | null; pricedAt: string | null } | null>}
 */
export async function getStoredCardPriceByName(playerName) {
 ...
 */
export async function seedCardPriceUrl({ playerId, playerName, productUrl, cardLabel, ungradedCadOverride }) {
  const id = String(playerId ?? "").trim();
  const url = String(productUrl ?? "").trim();
  if (!id || !url) return { ok: false, ungradedCad: null, error: "playerId/url requis" };

  const override = Number(ungradedCadOverride);
  const ungradedCad = Number.isFinite(override) && override > 0
    ? override
    : await fetchUngradedCadFromUrl(url);
