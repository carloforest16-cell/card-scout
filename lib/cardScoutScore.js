...
function scoreCachePath(playerId) {
  // v6 = refonte pondérations + hype/marché/liquidité (anti triple-comptage) → invalide v5
  return `scores/ai-score-v6-${playerId}.json`;
}
...
function scoreMarketValue(ebayMedianPriceCad, dealGapPct = null) {
  // Refonte v6 : le facteur Marché mesure si la carte est une AUBAINE (dispo
  // sous sa juste-valeur de cohorte), pas si elle est « pas chère » en absolu.
  // Une carte chère mais à son juste prix ne doit plus être pénalisée (cas des
  // gros noms), et une carte commune bon marché ne doit plus gonfler le score.

  // 1) Le prix absolu ne fait qu'un léger nudge (±1), pour ne pas écraser le
  // score d'un joueur populaire juste parce que ses cartes valent plus cher.
  let base = 5.5; // neutre quand le prix est inconnu
  if (ebayMedianPriceCad != null && Number.isFinite(Number(ebayMedianPriceCad))) {
    const m = Number(ebayMedianPriceCad);
    if (m < 15) base = 6.5;
    else if (m < 50) base = 6;
    else if (m < 150) base = 5.5;
    else if (m < 300) base = 5;
    else base = 4.5;
  }

  // 2) Le vrai signal d'investissement : l'écart à la juste-valeur. Sous la cote
  // = opportunité (bonus) ; au-dessus = on paie une prime (malus).
  const gap = Number(dealGapPct);
  if (Number.isFinite(gap)) {
    if (gap <= -30) base += 3;
    else if (gap <= -20) base += 2;
    else if (gap <= -10) base += 1;
    else if (gap <= -3) base += 0.5;
    else if (gap >= 30) base -= 2;
    else if (gap >= 15) base -= 1;
  }
