 ...
 */
/**
 * Poids des facteurs (v7.1). Total = 1.00.
 *
 * v7 → v7.1 (2026-06-21) :
 *  - teamContext activé à 0.04 (NHL standings, marché Canada/USA, demande locale)
 *  - hype réduit de 0.11 → 0.07 (overlap avec teamContext sur les marchés forts)
 *
 * Dérivé de `SCORE_WEIGHTS` (lib/cardScoutScoreMath.js) — source unique de
 * vérité partagée avec l'UI (tâche 3.1 du plan, plusieurs affichages avaient
 * dérivé avec des valeurs différentes). Ne pas modifier ici : éditer
 * SCORE_WEIGHTS.
 * @type {Record<string, number>}
 */
export const FACTOR_WEIGHTS = SCORE_WEIGHTS_BY_KEY;

const SYSTEM_PROMPT = `Tu es un expert en cartes de sport NHL et en analyse de marché.
Les 7 facteurs du Card Metrics Score sont DÉJÀ calculés mathématiquement côté serveur.
...
/**
 * Date du dernier match joué (régulier OU playoffs) — utilisé pour mesurer la
 * fraîcheur du signal momentum/teamContext en hors-saison.
 *
 * Stratégie :
 *  1. Si game log dispo, prend la date la plus récente (gameLog est trié desc).
 *  2. Sinon, fallback sur last5Games[0].gameDate du landing.
 *  3. Si playoff signal présent et récent, prend le max entre les deux.
 *
 * @param {Record<string, unknown>} landingData
 * @param {object | null} gameLog
 * @param {{ monthsSinceEnd?: number; season?: string } | null} playoffs
 * @returns {number | null} jours écoulés depuis le dernier match (entier ≥ 0)
 */
function computeDaysSinceLastGame(landingData, gameLog, playoffs) {
  const candidates = [];

  // 1. Game log (saison régulière, trié desc)
  ...
  // 2. last5Games du landing (fallback)
  if (Array.isArray(landingData?.last5Games) && landingData.last5Games.length > 0) {
    const d = landingData.last5Games[0]?.gameDate;
    if (typeof d === "string") {
      const ts = Date.parse(d);
      if (Number.isFinite(ts)) candidates.push(ts);
    }
  ...
  // 3. Playoffs récents : estimer la fin via la saison (~15 juin de l'année EEEE)
  if (playoffs?.season && typeof playoffs.season === "string" && playoffs.season.length === 8) {
    const endYear = Number(playoffs.season.slice(4, 8));
    if (Number.isFinite(endYear)) {
      // Si le run est encore en cours (monthsSinceEnd < 0), le joueur joue MAINTENANT.
      const months = Number(playoffs.monthsSinceEnd);
      if (Number.isFinite(months) && months < 0) {
        candidates.push(Date.now());
      } else {
        // Sinon, prend la date nominale de fin (15 juin de EEEE) comme proxy.
        candidates.push(new Date(endYear, 5, 15).getTime());
      }
    }
  }
  ...
  const days = Math.floor((Date.now() - mostRecent) / (24 * 60 * 60 * 1000));
  return Math.max(0, days);
}

/**
 * Facteur de fraîcheur (1.0 → 0.0) appliqué aux scores qui dépendent de l'activité
 * récente du joueur (momentumDetailed, teamContext). En hors-saison, ces signaux
 * deviennent périmés et doivent retomber vers neutre (5).
 *
 *   0–7 jours   : 1.0 (signal frais — joueur actif, régulier ou playoffs)
 *   7–45 jours  : décroissance linéaire 1.0 → 0.0
 *   45+ jours   : 0.0 (signal périmé)
 *
 * @param {number | null} days
 * @returns {number}
 */
function freshnessFactor(days) {
  if (days == null || !Number.isFinite(days)) return 1; // pas de donnée → neutre (n'altère pas)
  if (days <= 7) return 1;
  if (days >= 45) return 0;
  return 1 - (days - 7) / 38;
}
...
/**
 * Applique la fraîcheur à un score qui doit converger vers 5 (neutre) quand
 * il devient stale. Garde le score plein si frais, retombe vers 5 si périmé.
 *
 * @param {number} rawScore
 * @param {number | null} days
 * @returns {number}
 */
function decayedTowardNeutral(rawScore, days) {
  const f = freshnessFactor(days);
  if (f >= 1) return rawScore;
  return Math.round((rawScore * f + 5 * (1 - f)) * 10) / 10;
}
...
export function buildScorePayloadFromLanding(playerId, data, gameLog = null) {
  ...
  const daysSinceLastGame = computeDaysSinceLastGame(data, gameLog, playoffs);
  ...
  // de 3 ans. Seuil prudent : 2-3 ans peuvent correspondre à une blessure
  // longue (cas Landeskog 2022-25). 4 ans+ sans NHL → on bascule sur le mode
  // légende basé sur la carrière.
  ...
  const isRetired =
    lastPlayedYear != null && currentYear - lastPlayedYear > 3;
...
function buildRetiredFactorScores(payload, ebayMedianPriceCad, ebayListingCount, ebayDealGapPct) {
  ...
  // Upside vintage : pour les retraités, l'upside vient du legacy (HOF, vintage
  // appreciation, anniversaires/jersey retirements). Un journeyman retraité → 3.
  // Une légende → 6 (les cartes vintage prennent de la valeur lentement mais sûrement).
  let upsideRet = 3;
  if (Number.isFinite(careerPts)) {
    if (careerPts >= 1500) upsideRet = 7;   // GOAT tier — vintage premium
    else if (careerPts >= 1000) upsideRet = 6;
    else if (careerPts >= 700) upsideRet = 5;
    else if (careerPts >= 400) upsideRet = 4;
  }
  ...
  return {
    ...
    risk: 9, // retraité = aucun risque futur (blessure/déclin/scandale rare)
    teamContext: 5, // standings actuels sans pertinence
...
export function computeFactorScores(
  ...
  if (payload?.isRetired) {
    ...
    scores = {
      ...
      momentumDetailed: decayedTowardNeutral(
        scoreMomentumDetailed(
          payload?.momentumWindows ?? null,
          payload?.recentForm,
          cs.pointsPerGame
        ),
        payload?.daysSinceLastGame ?? null
      ),
...
const ENRICHMENT_META_KEYS = [
  "marketDiscrepancyMeta",
  "teamContextMeta",
  "catalystsMeta",
  "socialAttentionMeta",
];
...
/**
 * Un score est "enrichi" quand ses 4 sous-scores avancés (marketDiscrepancy,
 * teamContext, catalysts, socialAttention) ont été calculés — ce que le cron
 * hebdo `recompute-scores` ne fait JAMAIS (fastMode) et que seul le cron
 * quotidien `enrich-scores` (lib/scoreEnrichment.js) ajoute après coup.
 * Tâche 3.2 du plan — badge de complétude.
 * @param {Record<string, unknown> | null | undefined} factors
 * @returns {boolean}
 */
export function deriveEnrichmentStatus(factors) {
  if (!factors || typeof factors !== "object") return false;
  return ENRICHMENT_META_KEYS.every((key) => factors[key] != null);
}
...
export async function scoreCardScoutWithClaude(
  ...
  // fastMode: skip all async sub-scores + DeepSeek. Used by the cron to stay
  // within Vercel's 300s limit. Individual player pages use full mode.
    ...
    if (!payload?.isRetired && payload?.teamAbbrev) {
      try {
        ...
        if (teamContextMeta && Number.isFinite(teamContextMeta.score)) {
          // Décroissance hors-saison : les standings de la saison finie ne
          // reflètent plus la trajectoire actuelle de l'équipe.
          scores.teamContext = decayedTowardNeutral(
            teamContextMeta.score,
            payload?.daysSinceLastGame ?? null
          );
        }
      } catch {
  ...
  // fastMode: return math-only score without DeepSeek adjustment.
  if (fastMode) {
    const score = Math.round(Math.min(10, Math.max(0, weightedScore)) * 10) / 10;
    return {
      ok: true,
      score,
      verdict: score >= 7 ? "Acheter" : score >= 5 ? "Surveiller" : "Passer",
      reasoning: null,
      factors: baseFactors,
      tier: tierFromScore(score),
      verdictTone: verdictTone(score >= 7 ? "Acheter" : score >= 5 ? "Surveiller" : "Passer"),
      error: null,
      enriched: false,
      computedAt: nowIso,
    };
  ...
  const fail = (error) => ({
    ...
    tier: "unknown",
    error,
    enriched: deriveEnrichmentStatus(baseFactors),
    computedAt: nowIso,
  });
  ...
  // Repart de baseFactors (déjà enrichi des *Meta ci-dessus si !fastMode) plutôt
  // que de reconstruire via buildFactorsOutput — sinon les 4 sous-scores avancés
  // calculés un peu plus haut sont silencieusement perdus (bug trouvé tâche 3.2 :
  // scores/weightedScore identiques, seul scoreAdjustment doit s'ajouter ici).
  const factors = {
    ...baseFactors,
    weightedScore: Math.round(weightedScore * 1000) / 1000,
  };
  if (Number.isFinite(scoreAdjustment)) {
    factors.scoreAdjustment = Math.round(scoreAdjustment * 1000) / 1000;
  }
  ...
  const result = {
    ...
    error: null,
    enriched: deriveEnrichmentStatus(factors),
    computedAt: nowIso,
  };
