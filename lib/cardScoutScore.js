...
function scoreCachePath(playerId) {
  // v5 = mode légende pour les joueurs retraités (carrière vs saison) → invalide v4
  return `scores/ai-score-v5-${playerId}.json`;
}
...
function scoreMarketValue(ebayMedianPriceCad, dealGapPct = null) {
  // 1) Accessibilité : un prix d'entrée bas élargit la demande.
  let base;
  if (ebayMedianPriceCad == null || !Number.isFinite(Number(ebayMedianPriceCad))) {
    ...
    else if (m < 35) base = 6.5;
    else if (m < 75) base = 5;
    else if (m < 150) base = 3.5;
    else base = 2;
  }

  // 2) Précision par carte : la carte phare est-elle dispo SOUS sa juste-valeur
  // de cohorte ? Un écart négatif = vraie opportunité d'achat actuelle.
  const gap = Number(dealGapPct);
  if (Number.isFinite(gap)) {
    if (gap <= -30) base += 1.5;
    else if (gap <= -15) base += 1;
    else if (gap <= -5) base += 0.5;
  }
...
export function computeFactorScores(
  ...
  const scores = payload?.isRetired
    ? buildRetiredFactorScores(payload, ebayMedianPriceCad, ebayListingCount, ebayDealGapPct)
    : {
