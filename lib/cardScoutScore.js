...
function scoreCachePath(playerId) {
  // v2 = recalibrage des courbes (perf/momentum/âge) → invalide l'ancien cache
  return `scores/ai-score-v2-${playerId}.json`;
}
...
function scorePerformance(ptsPerGame) {
  ...
  // Calibré sur la distribution réelle NHL : élite ≈ 1.2+ pts/match,
...
function scoreMomentum(currentSeason, lastSeasons) {
  ...
  const gpCurrent = Number(currentSeason?.gamesPlayed);
  const prev = Array.isArray(lastSeasons) ? lastSeasons[0] : null;
  const prev2 = Array.isArray(lastSeasons) ? lastSeasons[1] : null;
  const ppgPrev = Number(prev?.pointsPerGame);

  // Recrue / pas d'historique exploitable : juger sur la production actuelle.
  // Capte le cas « jeune qui débarque et performe » même sans saison passée.
  if (!Number.isFinite(ppgPrev) || ppgPrev <= 0) {
    if (!Number.isFinite(ppgCurrent)) return 6;
    if (ppgCurrent >= 0.9) return 10;
    if (ppgCurrent >= 0.65) return 8;
    if (ppgCurrent >= 0.4) return 6;
    return 4;
  }

  if (!Number.isFinite(ppgCurrent)) return 5;

  // Variation PPG plutôt que points bruts → robuste aux blessures
  // (un joueur ayant raté 20 matchs n'est plus puni à tort).
  const delta = (ppgCurrent - ppgPrev) / ppgPrev;
  let s;
  if (delta >= 0.25) s = 10;
  else if (delta >= 0.12) s = 8.5;
  else if (delta >= 0.03) s = 6.5;
  else if (delta >= -0.05) s = 5;
  else if (delta >= -0.15) s = 3.5;
  else s = 2;

  // Progression continue sur 2 saisons → bonus de tendance
  const ppgPrev2 = Number(prev2?.pointsPerGame);
  if (
    Number.isFinite(ppgPrev2) && ppgPrev2 > 0 &&
    ppgPrev > ppgPrev2 && ppgCurrent > ppgPrev
  ) {
    s = Math.min(10, s + 1);
  }

  // Début de saison chaud : peu de matchs mais forte cadence → joueur en feu
  if (
    Number.isFinite(gpCurrent) && gpCurrent >= 5 && gpCurrent < 25 &&
    ppgCurrent >= 0.7
  ) {
    s = Math.min(10, s + 1);
  }
