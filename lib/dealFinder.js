...
export async function getEbayMedianAndCountForPlayer(playerName) {
  ...
  for (const c of qualified) {
    if (c.fairValueCad > 0 && c.minPriceCad != null) {
...
export async function buildInvestmentIntelligenceFromListings(
  ...
  // Juste-valeur par cohorte, calculée sur TOUT le set filtré (cohortes denses),
  // puis rattachée à chaque carte affichée (la moins chère de son groupe).
  const fairMap = computeFairValueByFingerprint(filtered);
  const scoredWithFair = scored.map((row) => {
    const key = cohortKeyForTitle(row.title);
    const fv = key ? fairMap.get(key) : null;
    const fairValueCad = fv?.fairValueCad ?? null;
    const dealDeltaPct =
      fairValueCad != null && Number(row.price) > 0
    ...
    return {
      ...
      dealDeltaPct,
      fairValueConfidence: fv?.confidence ?? "insufficient",
      fairValueComps: fv?.comps ?? 0,
    };
  });

  const buyListings = scoredWithFair.filter(
    (row) =>
