...
/**
 * Médiane trimmée (sans outliers) ; ne retient que les deals ≥ 20 % sous la médiane.
...
function summarizeGroupDeals(bucket, type, meta = {}) {
  ...
  const dealThreshold = marketPrice * DEAL_BELOW_MEDIAN_RATIO;
  const dealRows = dealPool.filter((row) => row.priceCad <= dealThreshold);
