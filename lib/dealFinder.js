...
import { PACK_BOX_RE, CUSTOM_ART_RE, isPackOrLotTitle } from "@/lib/titleFilters";
...
// Cache persistant des recherches (Supabase cache_generic), pour TOUS les
// joueurs. Avant : seul Cole Caufield avait un cache durable ; les autres ne
// vivaient qu'en mémoire du process — sur Vercel (process éphémères) presque
// chaque recherche repartait de zéro (~8 s, audit 2026-09-23).
//  - < 2 h : servi tel quel ;
//  - 2 h à 24 h : servi immédiatement (avec sa vraie heure de calcul, `scoredAt`)
//    et recalculé en arrière-plan pour la prochaine visite ;
//  - > 24 h : recalcul avant réponse.
const DEALS_CACHE_PREFIX = "deals-v2:";
const DEALS_CACHE_STALE_MAX_MS = 24 * 60 * 60 * 1000;
/** Recalculs en arrière-plan en cours (par process) — évite les doublons. */
const dealsRefreshing = new Set();
 ...
 * @param {"EBAY_CA" | "EBAY_US"} marketplace
 */
function dealsCacheKey(name, mode, marketplace) {
  // Le marché fait partie de la clé : avant, une recherche eBay US pouvait
  // recevoir le résultat eBay CA mis en cache.
  return `${name.toLowerCase()}:${mode}:${marketplace}`;
}
 ...
 */
async function readDealsCache(key) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .eq("key", DEALS_CACHE_PREFIX + key)
      .maybeSingle();
    if (error) throw new Error(error.message);
    if (!data?.payload) return null;
    const fetchedAt = new Date(data.fetched_at).getTime();
    return Number.isFinite(fetchedAt) ? { fetchedAt, result: data.payload } : null;
  } catch (err) {
    console.error("[dealFinder] lecture du cache de recherche échouée:", err?.message ?? err);
    return null;
  }
}
 ...
 */
async function writeDealsCache(key, entry) {
  const supabase = getSupabase();
  if (!supabase) return;
  try {
    const { error } = await supabase.from("cache_generic").upsert(
      {
        key: DEALS_CACHE_PREFIX + key,
    ...
    console.error("[dealFinder] écriture du cache de recherche échouée:", err?.message ?? err);
  }
}
...
// « CUSTOM » a migré vers CUSTOM_ART_RE (lib/titleFilters.js) — source unique
// partagée avec le harnais, aux côtés des autres marqueurs custom/art (B3).
const TITLE_EXCLUDE_RE =
  /\b(REPRINT|FAKE|LOT|BULK|COPY|PROXY|REPRODUCTION)\b|\bLOT\s+OF\b/i;
...
const TITLE_EXCLUDE_NON_CARD_RE =
  /\b(?:Poster|Photo|8x10|11x17|Canvas\s+Print|Framed|Plaque|Bobblehead|Figurine|Magazine|Sticker|Decal|Magnet|Puck|Pennant|Giveaways?|Arena\s+Giveaway|Stadium\s+Giveaway|Checklist|Jumbo|Jumbos|Oversized?)\b|\bPrint\b(?!ing)(?!\s+Run)/i;
...
export function shouldExcludeTitle(title, cardMode = CARD_MODE_RAW) {
  ...
  if (CUSTOM_ART_RE.test(title)) return true; // customs / hand-made / art (B3)
...
function mapRawListing(item) {
  ...
  // Trois états distincts pour `shippingCad` (convention documentée dans
  // `lib/ebayServer.js`) : 0 = gratuit annoncé, > 0 = connu, null = INCONNU.
  // L'API *search* d'eBay omet le port sur ~15 % des annonces ; le compter
  // comme gratuit les faisait paraître moins chères et donc remonter en tête.
  let shipCad = null;
  ...
  const priceCad = Math.round((basePriceCad + (shipCad ?? 0)) * 100) / 100;
  ...
  // `legacyItemId` sert à retrouver le vrai port via l'API *item detail* quand
  // la recherche ne l'annonce pas (voir `resolveMissingShipping`).
  const legacyItemId = item?.legacyItemId != null ? String(item.legacyItemId) : null;
  return { title, priceCad, basePriceCad, shippingCad: shipCad, legacyItemId, url, imageUrl };
}

// Récupération du port manquant : plafonds volontairement bas. Chaque annonce
// non résolue coûte un appel eBay supplémentaire, et ~15 % des annonces sont
// concernées (soit ~12 sur une recherche de 80).
const SHIPPING_RESOLVE_MAX = 25;
const SHIPPING_RESOLVE_CONCURRENCY = 5;

/**
 * L'API *search* d'eBay omet le port sur ~15 % des annonces ; l'API *item
 * detail* le connaît environ une fois sur deux. On va donc le chercher pour les
 * annonces concernées, puis on RECALCULE leur coût total.
 *
 * Les annonces dont le port reste inconnu après cet appel gardent
 * `shippingCad === null` — il n'est ni corrigé ni inventé, et `assignDealRanks`
 * les écarte du badge « meilleur deal » pour qu'une annonce au coût réel inconnu
 * ne puisse pas se prétendre la meilleure affaire.
 *
 * Mute les objets en place (ce sont des annonces fraîchement mappées).
 * @param {Array<object>} listings
 * @param {{ max?: number }} [options]
 * @returns {Promise<{ attempted: number; resolved: number }>}
 */
export async function resolveMissingShipping(listings, { max = SHIPPING_RESOLVE_MAX } = {}) {
  const targets = (Array.isArray(listings) ? listings : [])
    .filter((l) => l && l.shippingCad == null && l.legacyItemId)
    .slice(0, max);
  if (targets.length === 0) return { attempted: 0, resolved: 0 };

  let resolved = 0;
  for (let i = 0; i < targets.length; i += SHIPPING_RESOLVE_CONCURRENCY) {
    const chunk = targets.slice(i, i + SHIPPING_RESOLVE_CONCURRENCY);
    await Promise.all(
      chunk.map(async (l) => {
        try {
          const detail = await fetchEbayItemByLegacyId(l.legacyItemId);
          if (detail && detail.shippingCad != null) {
            const baseCad = l.basePriceCad ?? l.priceCad;
            l.shippingCad = detail.shippingCad;
            l.priceCad = Math.round((baseCad + detail.shippingCad) * 100) / 100;
            resolved++;
          }
        } catch (err) {
          // Jamais en silence (garde-fou : un catch muet a servi un cache
          // périmé 2 semaines en juin).
          console.error(
            `[dealFinder] resolveMissingShipping failed for ${l.legacyItemId}:`,
            err?.message ?? err
          );
        }
      })
...
export function cohortKeyForTitle(title, knownLastName = null) {
  ...
  if (
    feat &&
    (feat.cardCode || feat.parallelColor || feat.parallelTag || feat.namedInsert || feat.grade || feat.printRun || feat.variantTag)
  ) {
    const parts = [
      who || group,
      year,
      feat.variantTag,
      feat.namedInsert,
      // Le numéro de carte DOIT faire partie de la clé : il déclenche ce repli
      // mais n'y figurait pas, si bien que Planet Metal #PM-24, Amped Up #AU-29
      // et la base #150 de Caufield 2021 partageaient « pf|caufield-2021 » et
      // la même cote (audit 2026-09-23).
      feat.cardCode ? `no${String(feat.cardCode).toLowerCase()}` : null,
      feat.parallelTag,
      feat.parallelColor,
...
export async function buildInvestmentIntelligenceFromListings(
  ...
  // Le port doit être résolu AVANT tout le reste : les cohortes de juste-valeur
  // comme les écarts « sous la cote » se calculent sur `priceCad`, et un port
  // manquant compté comme gratuit fausse les deux.
  try {
    const ship = await resolveMissingShipping(filtered);
    if (ship.attempted > 0) {
      console.log(
        `[dealFinder] port manquant : ${ship.resolved}/${ship.attempted} résolus via l'API detail`
      );
    }
  } catch (err) {
    console.error("[dealFinder] resolveMissingShipping a échoué en bloc:", err?.message ?? err);
  }
  ...
  const enrichedRows = base.listingRows.map((row) => {
    ...
    // scope "broad" : la médiane vient du repli élargi (autres cartes du joueur,
    // ex. toutes les autos tous sets confondus) → ce n'est PAS une cote de CETTE
    // carte, seulement une RÉFÉRENCE. On ne l'affiche jamais comme « Cote », on
    // n'en tire jamais de % ni de signal de deal (bug B2 : Suzuki auto MVP cotée
    // 60,50 $ « −44 % » depuis 34–112 $ d'autres autos).
    const isBroad = (fv?.fairValueScope ?? "exact") === "broad";
    const rawFair = fv?.fairValueCad ?? null;
    const fairValueCad = isBroad ? null : rawFair;
    const referenceValueCad = isBroad ? rawFair : null;
    // Le SIGNAL de deal (% + ranking Hottest) n'est émis que si la cohorte est
    // FIABLE (≥4 comps) ET de scope exact. 2-3 comps → estimé affiché mais sans
    // clamer "-X%" (sinon un médian bruité fait passer une carte chère pour une
    // aubaine).
    const trusted =
    ...
    return {
      ...
      fairValueCad,
      referenceValueCad,
      fairValueScope: isBroad ? "broad" : "exact",
      ...
      dealDeltaPct: trusted
        ...
        : null,
      // Une réf broad n'est jamais « high/medium » : au mieux « low ».
      fairValueConfidence: isBroad ? "low" : (fv?.confidence ?? "insufficient"),
      fairValueComps: comps,
      // Ne pas labelliser « ventes réelles » une référence broad (pas la carte).
      fairValueSource: isBroad ? null : (fv?.fairValueSource ?? null),
      referenceSource: isBroad ? (fv?.fairValueSource ?? null) : null,
      comps130: isBroad ? null : (Array.isArray(fv?.comps130) ? fv.comps130 : null),
      fairValueRange: isBroad ? null : (fv?.range130 ?? null),
      referenceRange: isBroad ? (fv?.range130 ?? null) : null,
      fairValueLastSale: fv?.lastSale130 ?? null,
      // Cote servie depuis un cache périmé parce que 130point était
      // indisponible. Remonté jusqu'à la carte : une cote datée s'affiche avec
      // sa date, jamais comme une cote fraîche.
      fairValueStale: fv?.fairValueStale === true,
      fairValueAsOf: fv?.fairValueAsOf ?? null,
      // Métadonnée de provenance normalisée (lib/marketValue.js) — additive,
      // pour les badges honnêtes de la tâche 2.2. Broad → source "none".
      marketValueMeta: normalizeFromFairMapEntry({
        fairValueCad,
        comps,
        fairValueSource: isBroad ? null : (fv?.fairValueSource ?? null),
        range130: isBroad ? null : (fv?.range130 ?? null),
  ...
  // Rang serveur stable (badge « MEILLEUR DEAL »/« TOP 3 ») — indépendant des
  // filtres client (B10).
  assignDealRanks(display);
...
/**
 * Attribue un rang STABLE (serveur) aux vrais deals actionnables, pour que le
 * badge « MEILLEUR DEAL »/« TOP 3 » ne dépende plus de l'index d'une liste
 * filtrée côté client (bug B10 : filtrer par équipe/prix déplaçait le badge, et
 * une cote faussée devenait « meilleur deal »). Un deal actionnable = cote
 * EXACTE fiable (percentOfMarket non null) + prix ≥ 5 % sous la cote + score
 * ≥ 7,5 + COÛT TOTAL CONNU. Mute les objets en place (ce sont des cartes
 * fraîches). Le #1 reçoit `isTopDeal` + `topDealReason` ; les 3 premiers
 * reçoivent `rank`.
 *
 * La condition « coût total connu » (`shippingCad != null`) écarte les annonces
 * dont eBay n'annonce pas le port, même après l'appel *item detail*. Sans elle,
 * leur prix sous-évalué gonflait leur écart « sous la cote » et les propulsait
 * en tête — le biais qui a fait couronner une Demidov « MEILLEUR CHOIX » à
 * 105,94 $ alors qu'elle coûtait 130,40 $ port compris.
 * @param {Array<object>} cards
 * @returns {Array<object>}
 */
export function assignDealRanks(cards) {
  const list = Array.isArray(cards) ? cards : [];
  for (const c of list) {
    // Repart d'un état propre (le cache pourrait contenir d'anciens flags).
    delete c.isTopDeal;
    delete c.rank;
  ...
  const eligible = list
    .filter(
      (c) =>
        ...
        Number(c.investmentScore) >= 7.5
    )
    .sort((a, b) => Number(a.dealDeltaPct) - Number(b.dealDeltaPct)); // plus négatif d'abord
  eligible.forEach((c, i) => {
    if (i < 3) c.rank = i + 1;
    if (i === 0) {
      c.isTopDeal = true;
      const savings = Math.round(Number(c.fairValueCad) - Number(c.price));
      c.topDealReason =
        savings > 0
          ? `${savings} $ sous la cote`
          : `${Math.abs(Number(c.dealDeltaPct))} % sous la cote`;
    }
  });
  return list;
}
    ...
    for (const type of getOrderedGroupKeys(typeBuckets)) {
      ...
      for (const row of bucket) {
        selected.push({
          title: row.title,
          price: row.priceCad,
          // null = port INCONNU même après l'appel detail. Propagé jusqu'au
          // client pour que la carte l'affiche et que le rang l'écarte.
          shippingCad: row.shippingCad ?? null,
          url: row.url,
...
function guardJunkVerdict(scored) {
  const title = String(scored.title ?? "");
  if (TITLE_EXCLUDE_PACK_BOX_RE.test(title)) {
    return {
      ...scored,
  ...
  // Défense en profondeur B3 : si un custom/art a franchi shouldExcludeTitle
  // (ex. via une variante d'écriture), il ne doit jamais ressortir en Acheter.
  if (CUSTOM_ART_RE.test(title)) {
    return {
      ...scored,
      investmentScore: Math.min(Number(scored.investmentScore) || 0, 3),
      verdict: "Passer",
      upside: "Faible",
      reason: "Carte custom / faite main — pas un actif officiel à coter.",
    };
  }
  return scored;
}
...
/**
 * Garde-fou B4 : on ne recommande JAMAIS « Acheter » sans référence de prix
 * fiable. `percentOfMarket == null` = pas de cohorte exacte fiable (ou scope
 * broad) → impossible de dire si le prix est bon. On ne dit pas « Acheter » à
 * l'aveugle (Demidov « Acheter » 7,5 avec « Cote indisponible ») : verdict
 * plafonné à « Chercher mieux », score ≤ 6,9, upside max « Moyen ». Aucune
 * exception : une carte rare est « à juger par soi-même », pas un achat certifié.
 * Le CTA client passe alors automatiquement à « Voir sur eBay » (isAcheter=false).
 * @template {{ percentOfMarket?: number | null; investmentScore?: number; verdict?: string; upside?: string }} T
 * @param {T} scored
 * @returns {T}
 */
function guardBuyWithoutFairValue(scored) {
  // La démo (sans eBay) n'a pas de percentOfMarket mais est explicitement
  // étiquetée « DÉMO » dans l'UI — ne pas la vider de ses « Acheter » fictifs.
  if (scored.scoreSource === "demo") return scored;
  if (scored.percentOfMarket != null) return scored;
  const score = Number(scored.investmentScore) || 0;
  const capped = Math.round(Math.min(score, 6.9) * 10) / 10;
  const isBuy = String(scored.verdict ?? "").toLowerCase().includes("acheter");
  return {
    ...scored,
    investmentScore: capped,
    verdict: isBuy ? "Chercher mieux" : scored.verdict,
    upside: scored.upside === "Fort" ? "Moyen" : scored.upside,
  };
}

// En dessous, « Acheter » n'a pas de sens pour un outil d'INVESTISSEMENT : le
// port et les frais pèsent autant que la carte, et la plus-value possible se
// compte en sous. Audit 2026-09-23 : les 2 seuls « Acheter » de Cole Caufield
// étaient des inserts Metal Universe à 3,95 $ et 4,95 $.
const MIN_BUY_TOTAL_CAD = 15;

/**
 * Plafonne à « Chercher mieux » une carte trop bon marché pour être un
 * investissement (prix livré < MIN_BUY_TOTAL_CAD). Même traitement que B4.
 * @template {{ price?: number; investmentScore?: number; verdict?: string; upside?: string; scoreSource?: string }} T
 * @param {T} scored
 * @returns {T}
 */
function guardBuyTooCheap(scored) {
  if (scored.scoreSource === "demo") return scored;
  const price = Number(scored.price);
  if (!Number.isFinite(price) || price >= MIN_BUY_TOTAL_CAD) return scored;
  if (!String(scored.verdict ?? "").toLowerCase().includes("acheter")) return scored;
  const score = Number(scored.investmentScore) || 0;
  return {
    ...scored,
    investmentScore: Math.round(Math.min(score, 6.9) * 10) / 10,
...
function attachInvestmentScores(flat, claudeResult, forceDemoHeuristic, playerContext = null) {
  ...
  if (forceDemoHeuristic || !claudeResult.ok || !claudeResult.scores?.length) {
    return flat.map((L) => {
      ...
      return guardJunkVerdict(guardBuyTooCheap(guardBuyWithoutFairValue(applyPlayerQualityCap({
        ...L,
...
async function fetchPlayerContextForDeals(playerName) {
  try {
    ...
    // Le test était INVERSÉ : il vérifiait que chaque mot du RÉSULTAT figure
    // dans la REQUÊTE, ce que satisfait n'importe quel nom plus court. Sur
    // « Ivan Demidov », l'API NHL renvoie « Ivan Ivan » (BOS) en premier et le
    // vrai Demidov en troisième — « ivan » + « ivan » étant tous deux présents
    // dans la requête, le mauvais joueur était retenu. Tout le contexte de
    // scoring (âge, équipe, pts/match, Card Metrics Score) venait donc d'un
    // autre joueur.
    // Et surtout : plus de repli `?? results[0]`. Aucun contexte vaut mieux
    // qu'un contexte faux — on ne présente pas la donnée d'un autre joueur.
    const norm = (s) =>
      String(s ?? "")
        .toLowerCase()
        .normalize("NFD")
    ...
    const match =
      // 1. Nom identique après normalisation — le cas nominal.
      results.find((r) => norm(r.name) === wanted) ??
      // 2. Sinon, chaque mot de la REQUÊTE doit être un mot entier du résultat
      //    (gère « Demidov » seul, ou un second prénom côté NHL).
      results.find((r) => {
        const words = norm(r.name).split(" ").filter(Boolean);
        return wantedWords.length > 0 && wantedWords.every((w) => words.includes(w));
      }) ??
      null;

    if (!match?.playerId) {
      console.error(
        `[dealFinder] aucun joueur NHL ne correspond à « ${playerName} » — contexte de scoring omis (candidats : ${results
          .map((r) => r.name)
          .join(", ")})`
      );
      return null;
    ...
    try {
      ...
      // Jamais en silence : sans ce score, `applyPlayerQualityCap` retombe sur
      // un plafond de 7,0 — sous le seuil de 7,5 du badge « meilleur deal »,
      // qui devient alors inatteignable sans la moindre trace.
      console.error(
        `[dealFinder] lecture player_scores échouée pour ${match.playerId}:`,
        err?.message ?? err
      );
    }
...
/**
 * Calcul complet (eBay + cotes + scoring IA), sans cache. `scoredAt` = heure
 * réelle du calcul, affichée par le client (« Scoré il y a X min »).
 * `cacheable: false` pour les erreurs et la démo sans jeton eBay : jamais mises
 * en cache partagé.
 * @returns {Promise<{ result: object; cacheable: boolean }>}
 */
async function computeDealFinderResult(name, mode, marketplace, options) {
  const token = await resolveEbayBearerToken();
  if (!token) {
    const mockData = await buildMockInvestmentIntelligence(name, mode);
    if (!mockData.listings?.length) {
      const empty = await buildInvestmentIntelligenceFromListings(name, [], 100, true, mode);
      if (!empty.ok) {
        return {
          cacheable: false,
          result: {
    ...
    return { cacheable: false, result: { ok: true, data: { ...mockData, scoredAt: Date.now() } } };
  }

  const [ebay, playerContext] = await Promise.all([
    fetchEbayHockeyCardListingsForPlayer(name, token, marketplace),
  ...
  return { cacheable: true, result: { ok: true, data: { ...intel.data, scoredAt: Date.now() } } };
}

/**
 * Calcule, puis met en cache (mémoire + Supabase) si le résultat est valide.
 * @returns {Promise<object>}
 */
async function computeAndCacheDeals(key, name, mode, marketplace, options) {
  const { result, cacheable } = await computeDealFinderResult(name, mode, marketplace, options);
  if (cacheable) {
    const entry = { fetchedAt: Date.now(), result };
    playerResultCache.set(key, entry);
    await writeDealsCache(key, entry);
  }
  return result;
}
...
export async function getDealFinderResult(
  ...
  const key = dealsCacheKey(name, mode, marketplace);

  if (!options.forceRefresh) {
    const mem = playerResultCache.get(key);
    if (mem && Date.now() - mem.fetchedAt < PLAYER_RESULT_CACHE_TTL_MS) return mem.result;

    const stored = await readDealsCache(key);
    if (stored) {
      const age = Date.now() - stored.fetchedAt;
      if (age < PLAYER_RESULT_CACHE_TTL_MS) {
        playerResultCache.set(key, stored);
        return stored.result;
      }
      if (age < DEALS_CACHE_STALE_MAX_MS) {
        // Réponse immédiate avec l'ancien résultat (daté honnêtement par
        // `scoredAt`), recalcul en arrière-plan pour la visite suivante.
        if (!dealsRefreshing.has(key)) {
          dealsRefreshing.add(key);
          runInBackground(() =>
            computeAndCacheDeals(key, name, mode, marketplace, options).finally(() => dealsRefreshing.delete(key))
          );
        }
        return stored.result;
      }
  ...
  return computeAndCacheDeals(key, name, mode, marketplace, options);
}
