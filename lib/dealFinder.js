...
/**
 * Qualifiers fréquents qui signalent une recherche typée (auto, young guns, gradée…).
 * Si présents, on bascule en mode "search" même si le nom est court.
 */
const SEARCH_QUALIFIER_RE =
  /\b(?:auto|autograph|young\s+guns?|ygr?|rookie|rc|psa|bgs|sgc|cgc|graded?|grad(?:é|e)e?|parallel|parall[èe]le|canvas|gold|silver|black|patch|jersey|relic|opc|o-pee-chee|sp|ud|upper\s+deck|clear\s+cut|allure|the\s+cup|spx|premier|allstar|all-star|numbered|num[ée]rot[ée]|future\s+watch|metal\s+universe|flair|parkhurst|fleer|opc\s+platinum|trilogy|exclusives|credentials|artifacts|printing\s+plate|high\s+gloss|black\s+diamond|exquisite|dominion)\b/i;

/**
 * Détecte l'intention de la recherche :
 * - "curated" : juste un nom de joueur (1-2 mots, pas de qualifier) → meilleurs deals seulement
 * - "search"  : nom + qualifier OU 3+ mots → tous les listings pertinents
 * @param {string} query
 * @returns {"curated" | "search"}
 */
export function detectSearchIntent(query) {
  const trimmed = String(query || "").trim();
  if (!trimmed) return "curated";
  if (SEARCH_QUALIFIER_RE.test(trimmed)) return "search";
  const tokens = trimmed.split(/\s+/).filter(Boolean);
  if (tokens.length > 2) return "search";
  return "curated";
}
...
const TITLE_EXCLUDE_NON_CARD_RE =
  /\b(?:Poster|Photo|8x10|11x17|Canvas\s+Print|Framed|Plaque|Bobblehead|Figurine|Magazine|Sticker|Decal|Puck|Pennant|Giveaways?|Arena\s+Giveaway|Stadium\s+Giveaway)\b|\bPrint\b(?!ing)(?!\s+Run)/i;

/**
 * Packs scellés, boxes, tins et lots multi-cartes — pas une carte précise à
 * investir. Ajouté après l'audit 2026-07-13 : un « Fat Pack 26 Cards » ou un
 * « 8 card PACK » à ~4 $ passait pour une aubaine à -92 % vs la cote d'une single
 * et remontait EN TÊTE des deals/enchères. Prudence faux positifs :
 * on exige le pluriel « cards » (≠ « Series 2 Card »), on scope « Break » à
 * box/case/group, et « (3) » n'est capté qu'en début de titre (un lot).
 */
const TITLE_EXCLUDE_PACK_BOX_RE =
  /\b(?:Fat\s*Pack|Retail\s*Pack|Blaster|Hobby\s*(?:Box|Pack)|Mega\s*Box|Factory\s*(?:Set|Sealed)|Sealed\s*(?:Pack|Box|Case)?|Card\s+(?:Pack|Lot)|Possible)\b|\b\d+\s*cards\b|\b(?:Box|Case|Group)\s+Break\b|\bTin\b|^\s*\(\d+\)/i;
...
export function cohortKeyForTitle(title) {
  ...
  if (fp?.fingerprint) return fp.fingerprint;

  // Fallback : ancien schéma "groupe|année|tirage" si le fingerprint échoue
  // (titre trop bruité, langue inattendue, etc.).
...
export function computeFairValueByFingerprint(listings) {
  ...
  for (const l of Array.isArray(listings) ? listings : []) {
    ...
    if (!key) continue;
    // La clé peut être :
    //   - nouveau fingerprint "name-year-type-set-..." (séparateur "-")
...
export async function buildInvestmentIntelligenceFromListings(
  ...
  let fairMap = computeFairValueByFingerprint(filtered);
  try {
    fairMap = await enrichFairMapWith130Point(fairMap, name);
  } catch {
    // fallback silencieux : on garde la fairMap eBay active
  }
