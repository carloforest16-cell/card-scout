...
- RÈGLE DE PRIX FIABLE : si "percentOfMarket" est absent ou null (aucune cote fiable pour CETTE carte précise), tu NE PEUX PAS mettre "Acheter" — on ne recommande jamais un achat sans savoir si le prix est bon. Verdict max "Chercher mieux". Une carte rare sans référence se juge à la main, elle ne se certifie pas "Acheter".
- "reason" : UNE phrase COURTE de MAX 12 mots au format [fait sur la carte] + [fait sur le prix], spécifique à cette annonce précise (le remplissage est interdit). Tu DOIS mentionner AU MOINS 2 éléments concrets tirés des données : le type de carte exact, l'écart vs cote, l'âge du joueur, le grade PSA/BGS, ou les stats. FORMAT DE L'ÉCART IMPOSÉ : toujours en pourcentage SOUS/AU-DESSUS de la cote, ex. "−17 % vs cote" (jamais "à 83 % de la cote" — le lecteur ne doit pas avoir à convertir). ABSOLUMENT INTERDIT — si tu écris l'un de ces mots tu as échoué : "attractif", "potentiel", "intéressant", "standard", "bon deal", "favorable", "à surveiller", "profil". EXEMPLES OBLIGATOIRES à imiter : "YG RC −17 % vs cote, Raymond 22 ans C EDM en progression" / "PSA 10 Young Guns −6 % vs cote, Caufield 23 ans 31B MTL" / "Auto /99 −23 % vs cote — Tkachuk post-séries FLA 24G en playoffs" / "Canvas SP −12 % vs cote, Crosby 36 ans déclin probable"`.trim();
...
function buildHeuristicReason(L) {
  ...
  // Vocabulaire UNIQUE (B13) : on parle en ÉCART à la cote (« −18 % vs cote »),
  // exactement comme le pill dl-card__delta — plus de « à 82 % de la cote » qui
  // obligeait le lecteur à convertir mentalement 19 % = −81 %.
  const delta = hasPct ? pct - 100 : NaN;
  const deltaStr = hasPct
    ? delta === 0
      ? " au prix de la cote"
      : ` ${delta > 0 ? "+" : "−"}${Math.abs(delta)} % vs cote`
    : "";
  ...
  if (grade) {
    if (hasPct && pct <= 90) return `${grade}${deltaStr} — prime de grade bien intégrée${year ? ` (${year})` : ""}.`;
    if (hasPct && pct >= 112) return `${grade}${deltaStr} — évaluer la prime vs alternatives raw.`;
    if (hasPct) return `${grade}${deltaStr} — carte gradée liquide, vérifier les comparables récents.`;
    return buildLowConfidenceReason(grade, L.fairValueCad, L.fairValueComps, year);
  }
  if (/Young Guns|YG/i.test(gt)) {
    if (hasPct && pct <= 82) return `Young Guns RC${deltaStr}${year ? ` (${year})` : ""} — fenêtre d'achat favorable.`;
    if (hasPct && pct >= 110) return `Young Guns RC${deltaStr} — patienter ou chercher mieux.`;
    if (hasPct) return `Young Guns RC${deltaStr}${year ? ` (${year})` : ""} — carte flagship liquide.`;
    return buildLowConfidenceReason("Young Guns RC", L.fairValueCad, L.fairValueComps, year);
  }
  if (/Auto|RPA|Patch/i.test(gt)) {
    if (hasPct && pct <= 88) return `Auto/patch${deltaStr} — actif rare sous-évalué.`;
    if (hasPct) return `Auto/patch${deltaStr} — actif rare, à comparer aux ventes récentes.`;
    return buildLowConfidenceReason("Auto/patch", L.fairValueCad, L.fairValueComps, year);
  }
  if (/Gradée|Graded/i.test(gt)) {
    if (hasPct && pct <= 90) return `Gradée${deltaStr} — bon rapport qualité/prix.`;
    if (hasPct) return `Gradée${deltaStr} — prime à valider vs versions non gradées.`;
    return buildLowConfidenceReason("Gradée", L.fairValueCad, L.fairValueComps, year);
  }
  if (hasPct && pct <= 78) return `${gt || "Carte"}${deltaStr} — opportunité sous le marché actif.`;
  if (hasPct && pct >= 118) return `${gt || "Carte"}${deltaStr} — explorer les alternatives moins chères.`;
  if (hasPct) return `${gt || "Carte"}${deltaStr} — dans la fourchette normale du marché.`;
