 ...
 * @param {{ title?: string; groupType?: string; percentOfMarket?: number }} L
 */
function buildHeuristicReason(L) {
  const title = String(L.title ?? "");
  const gt = String(L.groupType ?? "");
  const pct = Number(L.percentOfMarket);
  ...
  if (grade) {
    ...
    return `${grade}${pctStr} — carte gradée liquide, vérifier les comparables récents.`;
  }
  if (/Young Guns|YG/i.test(gt)) {
    ...
    return `Young Guns RC au prix standard${year ? ` (${year})` : ""} — carte flagship liquide.`;
  }
  if (/Auto|RPA|Patch/i.test(gt)) {
    if (hasPct && pct <= 88) return `Auto/patch${pctStr} de la cote — actif rare sous-évalué.`;
    return `Auto/patch — actif rare, vérifier la numérotation et la demande actuelle.`;
  }
  if (/Gradée|Graded/i.test(gt)) {
    if (hasPct && pct <= 90) return `Gradée${pctStr} de la cote — bon rapport qualité/prix.`;
    return `Gradée — prime à valider par rapport aux versions non gradées.`;
  }
  ...
  return `${gt || "Carte"} — données marché limitées, comparer manuellement.`;
}
...
export function mockInvestmentScores(listings, playerContext = null) {
  ...
  return listings.map((L) => {
    ...
    // Score final : blend prix + qualité joueur
    ...
    // Plafond de cohérence joueur (même règle que le prompt Claude, ligne ~52) :
    // le score d'investissement ne dépasse pas la qualité du joueur + 1.5. Un
    // rabais sur la carte d'un joueur faible ne fait pas une aubaine. L'audit
    // voyait un joueur AHL (Card Metrics 5.0) noté 8.7 = même score qu'une star.
    // Sans contexte joueur du tout → prudence, plafond 6.5.
    let finalScore = score;
    if (playerQuality != null) {
      finalScore = Math.min(finalScore, Math.round((playerQuality + 1.5) * 10) / 10);
    } else {
      finalScore = Math.min(finalScore, 6.5);
    }

    let verdict = "Chercher mieux";
    if (finalScore >= 7 && hasPct && pct <= 105) verdict = "Acheter";
    else if (finalScore < 4.5 || (hasPct && pct > 130)) verdict = "Passer";
    const upside = finalScore >= 7.5 ? "Fort" : finalScore >= 5.5 ? "Moyen" : "Faible";
