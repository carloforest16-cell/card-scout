...
export const HOTTEST_MIN_INVESTMENT_SCORE_RAW = 7.0;
export const HOTTEST_MIN_INVESTMENT_SCORE_GRADED = 5.5;
 ...
 * @param {"raw" | "graded" | string} cardMode
 */
export function getHottestMinInvestmentScore(cardMode) {
  return resolveMode(cardMode) === "graded"
    ? HOTTEST_MIN_INVESTMENT_SCORE_GRADED
    : HOTTEST_MIN_INVESTMENT_SCORE_RAW;
}
...
/**
 * Démo sans eBay : cartes fictives au-dessus du seuil mode pour remplir la section.
 * @param {"raw" | "graded"} cardMode
 * @returns {Array<object>}
 */
function buildMockHottestCards(cardMode) {
  const mode = resolveMode(cardMode);
  const minScore = getHottestMinInvestmentScore(mode);
  const labels = [
    "Connor McDavid",
    "Sidney Crosby",
    "Nathan MacKinnon",
  ...
  return labels.map((playerName, i) => ({
    ...
    investmentScore: minScore + i * 0.08,
...
/**
 * Agrège les stars NHL : eBay + scores en parallèle, filtre par seuil mode, top 6.
 * @param {"raw" | "graded" | string} [cardMode]
...
async function buildHottestDealsFresh(cardMode = "raw") {
  ...
  const cards = merged
    .filter(
      (c) =>
        Number(c.investmentScore) >= minScore &&
