...
// Filtres UX de la section « Hottest » : c'est la vitrine, donc on ne veut ni
// joueurs obscurs (Carlo 2026-07-16 : « Spencer Foo / Corson Ceulemans, who tf
// are those guys ») ni deals médiocres.
// - MIN_HOTTEST_PLAYER_SCORE : plancher sur le Card Metrics Score du joueur.
//   6.0 = Bedard, Celebrini, Slafkovský, McDavid, Stützle passent. Foo/Ceulemans
//   (CM ~5.1) sont éliminés.
// - MIN_HOTTEST_DEAL_SCORE : plancher sur l'investmentScore de la carte.
// Le tri final combine 60% investmentScore + 40% CardMetrics (les stars sont
// privilégiées à deal égal, mais un deal exceptionnel sur un joueur moyen peut
// encore remonter).
// Les CM en base sont actuellement compressés (McDavid stocké à 5.9, Stützle 5.9)
// donc 5.5 est le bon seuil pour garder les vraies stars tout en éliminant les
// vrais inconnus (Foo, Ceulemans à ~5.1). Un jour où la calibration CM sera fixée,
// remonter à 6.0-6.5.
const MIN_HOTTEST_PLAYER_SCORE = 5.5;
const MIN_HOTTEST_DEAL_SCORE = 6.0;
const HOTTEST_STAR_WEIGHT = 0.4; // poids du CM dans le tri (1 - poids du deal)
...
async function buildHottestDealsFresh(cardMode = "raw") {
  ...
  // Double filtre vitrine : joueur pertinent (CM ≥ 6.0) ET deal solide (score
  // ≥ 6.5). Sans CM résolu on garde la carte (mieux qu'une section vide).
  const filtered = merged.filter((c) => {
    const cm = Number(c.cardScoutScore);
    const cmOk = !Number.isFinite(cm) || cm >= MIN_HOTTEST_PLAYER_SCORE;
    const dealOk = Number(c.investmentScore) >= MIN_HOTTEST_DEAL_SCORE;
    return cmOk && dealOk;
  });
  // Tri hybride : 60 % investmentScore + 40 % Card Metrics du joueur. Un très
  // bon deal sur un joueur moyen peut remonter, mais à deal égal la star gagne.
  const dealWeight = 1 - HOTTEST_STAR_WEIGHT;
  const rank = (c) =>
    Number(c.investmentScore) * dealWeight +
    (Number.isFinite(Number(c.cardScoutScore)) ? Number(c.cardScoutScore) : 5) * HOTTEST_STAR_WEIGHT;
