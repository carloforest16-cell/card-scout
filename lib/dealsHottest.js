...
// Rabais minimum (vs cote fiable) pour qu'une carte soit un « vrai deal » digne
// de la vitrine Hottest. −5 % = clairement sous le marché, pas du bruit.
const MIN_HOTTEST_DISCOUNT_PCT = -5;
...
const BLOB_CACHE_PATHNAMES = {
  raw: "hottest-deals-raw-cache-v8.json",
  graded: "hottest-deals-graded-cache-v8.json",
};
...
async function buildHottestDealsFresh(cardMode = "raw") {
  ...
  // CURATION RUTHLESS (refonte 2026-07-23, décision Carlo) : « Hottest Deals »
  // ne montre QUE de VRAIS deals. Avant, la section raclait 40 gros noms et
  // empilait leurs cartes — 70 % sans cote (« cote indisponible »), verdict
  // « Chercher mieux », prix sans référence : un dump eBay déguisé qui ne
  // donnait aucune confiance. Un deal = cote FIABLE (percentOfMarket non null,
  // scope exact) + prix réellement SOUS la cote (≥ 5 %) + score plancher. Quitte
  // à n'afficher que 5 vrais deals plutôt que 22 bidons.
  const realDeals = merged.filter(
    (c) =>
      Number(c.investmentScore) >= MIN_HOTTEST_DEAL_SCORE &&
      c.percentOfMarket != null &&
      c.dealDeltaPct != null &&
      Number(c.dealDeltaPct) <= MIN_HOTTEST_DISCOUNT_PCT
  );
  // Tri : les ventes réelles 130point d'abord (la preuve la plus solide), puis
  // par taille du rabais (écart le plus négatif = le meilleur deal en tête).
  const sorted = realDeals.sort((a, b) => {
    const a130 = a.fairValueSource === "130point" ? 1 : 0;
    const b130 = b.fairValueSource === "130point" ? 1 : 0;
    if (a130 !== b130) return b130 - a130;
    return Number(a.dealDeltaPct) - Number(b.dealDeltaPct);
  });
  ...
  // Rang serveur recalculé sur la sélection finale inter-joueurs : le badge
  // « MEILLEUR DEAL »/« TOP 3 » désigne LE meilleur deal de toute la vitrine,
  // pas le #1 d'un joueur (les rangs par-joueur d'assignDealRanks amont sont
  // caducs après merge + diversité).
  assignDealRanks(cards);
