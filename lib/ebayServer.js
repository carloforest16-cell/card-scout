 ...
 * L'API requiert un accès spécifique — retourne { available: false } si 403.
 *
...
export async function fetchSoldComps(query, options = {}) {
  ...
  // TODO: eBay Marketplace Insights requires elevated API access.
  // Current plan returns 403 "Insufficient permissions".
  // When access is granted, this function returns real sold comps.
  // Apply at https://developer.ebay.com/ → Production Keys → OAuth Scopes
  if (res.status === 403) {
    return { available: false };
  }
