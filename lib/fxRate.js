...
async function fetchRateFromValet() {
  const res = await fetch(VALET_URL, { cache: "no-store", signal: AbortSignal.timeout(8000) });
...
export async function refreshFxRateIfStale() {
  ...
  refreshInFlight = (async () => {
    try {
      ...
      // planter à cause d'un souci de taux de change. On logge quand même
      // pour que ce ne soit plus invisible (cf. bug prod de juin, tâche 2.2).
      console.error("[fxRate] refreshFxRateIfStale failed, keeping cached rate:", err?.message ?? err);
    } finally {
