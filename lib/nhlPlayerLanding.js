...
export async function fetchPlayerLanding(playerId) {
  ...
  try {
    ...
    console.error("[nhlPlayerLanding] fetchPlayerLanding failed:", err?.message ?? err);
    landingMemoryCache.set(id, { data: null, fetchedAt: Date.now() });
    return null;
  }
}
...
/**
 * Récupère le game log NHL d'un joueur pour une saison donnée (regular season).
 * Endpoint: /v1/player/{id}/game-log/{season}/2
 * @param {string|number} playerId
 * @param {string|number} seasonId — format 20252026
 * @returns {Promise<object | null>}
 */
export async function fetchPlayerGameLog(playerId, seasonId) {
  const key = `${playerId}-${seasonId}`;
  const hit = gameLogMemoryCache.get(key);
  if (hit && Date.now() - hit.fetchedAt < GAME_LOG_MEMORY_TTL_MS) {
    return hit.data;
  }

  try {
    const res = await fetch(`${LANDING_BASE}/${playerId}/game-log/${seasonId}/2`, {
      headers: {
        Accept: "application/json",
        "User-Agent": "CardMetrics/1.0",
      },
    ...
    if (!res.ok) {
      gameLogMemoryCache.set(key, { data: null, fetchedAt: Date.now() });
      return null;
    }
    const data = await res.json();
    gameLogMemoryCache.set(key, { data, fetchedAt: Date.now() });
    return data;
  } catch (err) {
    console.error("[nhlPlayerLanding] fetchPlayerGameLog failed:", err?.message ?? err);
    gameLogMemoryCache.set(key, { data: null, fetchedAt: Date.now() });
    return null;
  }
}
