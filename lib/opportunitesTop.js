...
/**
 * @param {object} o
 * @param {Array<{ playerId: number; playerName: string; team: string; headshotUrl: string }>} top25
 */
function resolveCandidateFromOpportunity(o, top25) {
  const claudeId = Number(o?.playerId);
  const claudeName = String(o?.playerName ?? "")
    .trim()
    .toLowerCase();

  if (Number.isFinite(claudeId)) {
    const byId = top25.find((c) => c.playerId === claudeId);
    if (byId) return byId;
  }

  if (claudeName) {
    const exact = top25.find(
      (c) => c.playerName.trim().toLowerCase() === claudeName
    );
    if (exact) return exact;

    const lastWord = claudeName.split(/\s+/).pop();
    if (lastWord && lastWord.length >= 2) {
      const byLast = top25.find((c) => {
        const parts = c.playerName.trim().toLowerCase().split(/\s+/);
...
/**
 * @param {object} raw
 * @param {Array<{ playerId: number; playerName: string; team: string; headshotUrl: string; age?: number; ptsPerGame?: number; goals?: number; points?: number }>} top25
 */
function normalizeClaudePayload(raw, top25) {
  ...
  const normalized = opportunities.slice(0, TOP_OPPORTUNITIES_RETURN).map((o, i) => {
    const candidate = resolveCandidateFromOpportunity(o, top25);
    const id = candidate?.playerId ?? Number(o?.playerId);
    ...
    return {
      ...
      playerName: String(
        candidate?.playerName ?? o?.playerName ?? "—"
      ).trim(),
      team: String(candidate?.team ?? o?.team ?? "—").trim(),
      ...
      headshotUrl: candidate?.headshotUrl ?? null,
    };
  });
...
Retourne exactement ce JSON :
{
  ...
  const payload = normalizeClaudePayload(obj, top25);
