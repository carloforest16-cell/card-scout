...
function mergeNarrativeWithScoredTop8(claudeRaw, scoredTop8) {
  ...
  const opportunities = scoredTop8.map((c, i) => {
    ...
    return {
      ...
      reasoning: String(
        narrative.reasoning ??
          `${c.playerName} figure dans le top 8 Card Metrics Score pour la saison 2024-25.`
      ).trim(),
  ...
  return {
    ...
    analysisNote: String(
      claudeRaw?.analysisNote ??
        "Top 8 s̩lectionn̩s et class̩s par Card Metrics Score (7 facteurs + eBay)."
    ).trim(),
    opportunities,
    mocked: false,
  };
...
async function enrichTop8WithClaudeScores(top8) {
  ...
  const enriched = await Promise.all(
    top8.map(async (c) => {
      ...
      const scored = await scoreCardMetricsWithClaude(
        scorePayload,
        medianPriceCad,
        listingCount
      );
