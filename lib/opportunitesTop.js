...
import { ACTIVE_FILTER_MARGIN, keepActivePlayers } from "@/lib/playerScores";
...
Les 8 joueurs suivants ont DÉJÀ été sélectionnés et classés par le Card Metrics Score (7 facteurs mathématiques + marché eBay). Tu ne choisis PAS qui figure dans la liste et tu ne modifies PAS les scores.
...
async function writeOpportunitesCacheToSupabase(entry) {
  const supabase = getSupabaseAdmin();
  const { error } = await supabase.from("cache_opportunites").upsert({
    key: SUPABASE_CACHE_KEY,
    fetched_at: new Date(entry.fetchedAt).toISOString(),
    payload: entry.payload,
  }, { onConflict: "key" });
  if (error) {
    console.error("[opportunitesTop] writeOpportunitesCacheToSupabase failed:", error.message ?? error);
  }
}
...
function buildMockTopPayload(scoredTop8) {
  ...
  return {
    lastUpdated: now,
    analysisNote:
      "Top 8 sélectionnés par Card Metrics Score (7 facteurs + eBay)  -  activez CS_CLAUDE_KEY pour les textes Claude.",
    opportunities,
    mocked: true,
  };
}
...
function mergeNarrativeWithScoredTop8(claudeRaw, scoredTop8) {
  ...
  const opportunities = scoredTop8.map((c, i) => {
    ...
    return {
      ...
      headline: String(
        narrative.headline ??
          `Profil ${c.ptsPerGame} pts/match  -  opportunité Card Metrics`
      ).trim(),
      reasoning: String(
        narrative.reasoning ??
  ...
  return {
    ...
    analysisNote: String(
      claudeRaw?.analysisNote ??
        "Top 10 sélectionnés et classés par Card Metrics Score (7 facteurs + eBay)."
    ).trim(),
    opportunities,
    mocked: false,
  };
...
async function analyzeTop8NarrativeWithClaude(scoredTop8) {
  ...
  console.log("[opportunitesTop] DeepSeek key présent:", !!apiKey, apiKey ? `(${apiKey.slice(0, 6)}...)` : "(vide)");
  if (!apiKey) {
    console.error("[opportunitesTop] analyzeTop8NarrativeWithClaude: DEEPSEEK_API_KEY manquant — retour mock payload");
    return buildMockTopPayload(scoredTop8);
  }

  const userPrompt = `Voici les 10 meilleures opportunités d'investissement cartes NHL, DÉJÀ classées par Card Metrics Score (ne change pas l'ordre ni la sélection) :
...
Retourne exactement ce JSON avec un objet par joueur (playerId obligatoire pour correspondance) :
{
  ...
  "analysisNote": (1 phrase : sélection basée sur Card Metrics Score 7 facteurs + eBay),
  "opportunities": [
    {
...
async function buildTopOpportunitesFromDb() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      // Marge : les joueurs retraités (Gretzky…) sont retirés juste après.
      .limit(TOP_OPPORTUNITIES_RETURN + ACTIVE_FILTER_MARGIN);
    if (error) {
      console.error("[opportunitesTop] buildTopOpportunitesFromDb query failed:", error.message ?? error);
      return null;
    }
    rows = (await keepActivePlayers(data ?? [])).slice(0, TOP_OPPORTUNITIES_RETURN);
  } catch (err) {
    console.error("[opportunitesTop] buildTopOpportunitesFromDb failed:", err?.message ?? err);
    return null;
  }
  if (!Array.isArray(rows) || rows.length < TOP_OPPORTUNITIES_RETURN) {
    console.error(`[opportunitesTop] buildTopOpportunitesFromDb: player_scores insuffisant (${rows?.length ?? 0} lignes, besoin de ${TOP_OPPORTUNITIES_RETURN}) — fallback batch NHL`);
    return null;
  }
