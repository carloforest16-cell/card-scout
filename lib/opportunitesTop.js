...
function buildMockTopPayload(scoredTop8) {
  ...
  return {
    lastUpdated: now,
    analysisNote:
      "Top 8 sÃ©lectionnÃ©s par Card Scout Score (7 facteurs + eBay)  -  activez CS_CLAUDE_KEY pour les textes Claude.",
    opportunities,
    mocked: true,
  };
}
...
function mergeNarrativeWithScoredTop8(claudeRaw, scoredTop8) {
  ...
  const opportunities = scoredTop8.map((c, i) => {
    ...
    return {
      ...
      headline: String(
        narrative.headline ??
          `Profil ${c.ptsPerGame} pts/match  -  opportunitÃ© Card Scout`
      ).trim(),
      reasoning: String(
        narrative.reasoning ??
...
async function analyzeTop8NarrativeWithClaude(scoredTop8) {
  ...
  const userPrompt = `Voici les 10 meilleures opportunitÃ©s d'investissement cartes NHL, DÃ‰JÃ€ classÃ©es par Card Scout Score (ne change pas l'ordre ni la sÃ©lection) :
