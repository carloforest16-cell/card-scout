...
// Nombre de lignes lues en plus pour compenser les joueurs non actifs retirés.
export const ACTIVE_FILTER_MARGIN = 20;
...
export async function getTopStoredScores(limit = 8) {
  ...
  const { data, error } = await supabase
    ...
    // Marge pour les retraités filtrés ci-dessous.
    .limit(limit + ACTIVE_FILTER_MARGIN);
  if (error || !Array.isArray(data)) return [];
  return (await keepActivePlayers(data)).slice(0, limit);
}

/**
 * Ne garde que les joueurs de l'effectif NHL actif (table `players`, synchro
 * hebdo). player_scores contient aussi des légendes retraitées (Gretzky, Roy,
 * Brodeur…) : leur fiche /player les score à la demande et le write-through
 * les persiste. Sans ce filtre, Wayne Gretzky sortait 7e du « Top 10
 * opportunités NHL » avec l'équipe « — » (audit 2026-09-23).
 * Ordre d'entrée conservé. En cas d'échec de lecture, renvoie les lignes
 * telles quelles plutôt que de vider les classements.
 * @template {{ player_id: string | number }} T
 * @param {T[]} rows
 * @returns {Promise<T[]>}
 */
export async function keepActivePlayers(rows) {
  if (!Array.isArray(rows) || rows.length === 0) return [];
  try {
    const supabase = getSupabaseAdmin();
    const ids = rows.map((r) => Number(r.player_id)).filter(Number.isFinite);
    const { data, error } = await supabase
      .from("players")
      .select("player_id")
      .eq("is_active", true)
    ...
    const active = new Set((data ?? []).map((p) => String(p.player_id)));
    return rows.filter((r) => active.has(String(r.player_id)));
  } catch (err) {
    console.error("[playerScores] keepActivePlayers — filtre des retraités ignoré:", err?.message ?? err);
    return rows;
  }
}
 ...
 * Passe par scoreCardScoutWithClaude en fastMode (données marché null) : aucun
 * appel externe, mais on récupère le détail par facteur (`result.factors`) —
 * indispensable pour que la page joueur affiche le graphique orbital / détaillé
 * du « pourquoi le score est X », pas juste le chiffre total.
 * @param {{ playerId: string; playerName: string; team: string; points: number; gamesPlayed: number }} player
 */
async function computeMathRowForPlayer(player) {
  ...
  const result = await scoreCardScoutWithClaude(payload, null, null, null, {
    skipCache: true,
    fastMode: true,
  });
  if (!result.ok || result.score == null) return null;
