...
import { normalizeFromPriceHistory } from "@/lib/marketValue";
  ...
  const enriched = await Promise.all(
    cards.map(async (card) => {
      ...
      return {
        ...
        priceFresh: isFresh,
        // Métadonnée de provenance normalisée (lib/marketValue.js) — pour les
        // badges honnêtes de la tâche 2.2. N'affecte rien d'existant, champ
        // additif : card_price_history vient d'annonces eBay actives snapshotées
        // (cron card-prices), donc toujours source "asking", jamais "sold".
        marketValueMeta: normalizeFromPriceHistory({
          valueCad: current,
          snapshotDateIso: snap?.snapshot_date ?? null,
        }),
      };
    })
  );
...
/**
 * `ilike` sert ici d'égalité insensible à la casse : on neutralise les jokers
 * SQL (« % », « _ ») et l'échappement, sinon un nom saisi « % » correspondrait
 * à n'importe quelle carte.
 * @param {unknown} value
 */
...
async function fetchLatestSnapshot(supabase, playerName, cardType, grade) {
  ...
  const query = supabase
    ...
    .limit(1);
  if (cardType) query.ilike("card_type", escapeLike(cardType));
  if (grade) query.ilike("grade", escapeLike(grade));
