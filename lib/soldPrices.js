...
function guardCardTypeAgainstTarget(sales, sampleTitle) {
  ...
  if (!target?.cardType) return { kept: sales, rejected: 0 };

  const kept = sales.filter((s) => {
    if (!s.title) return true;
    const fp = extractPartialFeatures(s.title);
    return !cardTypesConflict(target.cardType, fp?.cardType ?? null);
  });
...
export async function getSoldPriceStats(query, days = SOLD_WINDOW_DAYS, sampleTitle = null, options = {}) {
  ...
  // v5 : le garde-fou de famille de carte change les comps retenus. Les entrées
  // v4 contiennent des cotes empoisonnées (Young Guns Outburst mélangée à
  // Extended Outburst) — on ne peut pas les réutiliser.
  const cacheKey = `sold_130pt_v5:${normalizedQuery}:${days}d:${normalizedSample}`;
