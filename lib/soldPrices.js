...
/** Clé du compteur agrégé du garde-fou (tâche 3.2 de PLAN-DEAL-FINDER.md). */
const GUARD_STATS_KEY = "comp_guard_stats_v1";

/**
 * Accumule le taux de rejet du garde-fou pour pouvoir DÉCIDER, sur données
 * réelles, si le matching DeepSeek mérite de rester le chemin PRINCIPAL. Les
 * journaux le montraient déjà cas par cas (6/6, 24/25, 26/27 rejetés) mais
 * n'étaient pas agrégeables.
 *
 * Volontairement approximatif : lecture-modification-écriture sans verrou, donc
 * des incréments concurrents peuvent se perdre. C'est un indicateur de tendance,
 * pas une comptabilité — et le prix d'un compteur exact (table dédiée, upsert
 * atomique) ne se justifie pas ici. Ne jette jamais : la mesure ne doit pas
 * pouvoir casser une cote.
 * @param {number} evaluated — comps validés par l'IA, soumis au garde-fou
 * @param {number} rejected — comps que le garde-fou a écartés
 */
async function recordGuardOutcome(evaluated, rejected) {
  if (!Number.isFinite(evaluated) || evaluated <= 0) return;
  try {
    const prev = (await readJsonCache(GUARD_STATS_KEY)) ?? null;
    const next = {
      evaluated: (prev?.evaluated ?? 0) + evaluated,
      rejected: (prev?.rejected ?? 0) + rejected,
      batches: (prev?.batches ?? 0) + 1,
      since: prev?.since ?? new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
    await writeJsonCache(GUARD_STATS_KEY, next);
  } catch (err) {
    console.error("[soldPrices] compteur garde-fou non enregistré:", err?.message ?? err);
  }
}

/**
 * Lit le compteur agrégé du garde-fou. Exposé au panneau admin.
 * @returns {Promise<{ evaluated: number; rejected: number; batches: number; rejectionRatePct: number; since: string | null; updatedAt: string | null } | null>}
 */
export async function getCompGuardStats() {
  try {
    const s = await readJsonCache(GUARD_STATS_KEY);
    if (!s || !Number.isFinite(s.evaluated) || s.evaluated <= 0) return null;
    return {
      evaluated: s.evaluated,
      rejected: s.rejected ?? 0,
      batches: s.batches ?? 0,
      rejectionRatePct: Math.round(((s.rejected ?? 0) / s.evaluated) * 100),
      since: s.since ?? null,
      updatedAt: s.updatedAt ?? null,
    };
  } catch (err) {
    console.error("[soldPrices] lecture compteur garde-fou échouée:", err?.message ?? err);
    return null;
  }
}
...
async function filterSalesForTarget(sales, sampleTitle) {
  ...
  if (ai.ok) {
    ...
    if (rejected > 0) {
      console.error(
        `[soldPrices] garde-fou : ${rejected}/${aiKept.length} comps validés par l'IA rejetés (famille ou série différente) pour "${sampleTitle.slice(0, 70)}"`
      );
    }
    // Compteur agrégé — sans attendre : la mesure ne doit jamais ralentir une
    // cote ni la faire échouer.
    void recordGuardOutcome(aiKept.length, rejected);
...
export async function getSoldPriceStats(query, days = SOLD_WINDOW_DAYS, sampleTitle = null, options = {}) {
  ...
  if (primarySales === null) {
    // 130point est indisponible (timeout, 5xx). C'est la SEULE source de ventes
    // réelles : rendre EMPTY_STATS fait disparaître toutes les cotes du site dès
    // que le cache 24 h expire. Panne observée le 2026-07-30, 3 requêtes sur 3
    // en timeout à 30 s.
    // Une cote datée vaut mieux que pas de cote — à condition de dire son âge.
    // Le `stale` remonte jusqu'à l'UI, il n'est jamais présenté comme frais.
    if (cached?.data?.medianCad != null) {
      const ageHours = Math.round((Date.now() - cached.fetchedAt) / 3_600_000);
      console.error(
        `[soldPrices] 130point indisponible — cote PÉRIMÉE servie (${ageHours} h) pour "${normalizedQuery.slice(0, 60)}"`
      );
      return {
        ...cached.data,
        stale: true,
    ...
    console.error(
      `[soldPrices] 130point indisponible et aucun cache pour "${normalizedQuery.slice(0, 60)}" — aucune cote`
    );
    return { ...EMPTY_STATS };
  }
...
export async function enrichFairMapWith130Point(fairMap, playerName) {
  ...
  for (const { keys, stats } of results) {
    ...
    for (const key of keys) {
      ...
      // Une cote périmée (130point indisponible) ne peut jamais être « high » :
      // le marché a pu bouger depuis. Elle reste affichée, mais dégradée et
      // datée.
      if (stats.stale && confidence === "high") confidence = "medium";
      enriched.set(key, {
        ...existing,
        fairValueCad: stats.medianCad,
        comps: stats.count,
        confidence: stats.scope === "broad" ? "low" : confidence,
        fairValueStale: stats.stale === true,
        fairValueAsOf: stats.fetchedAtIso ?? null,
