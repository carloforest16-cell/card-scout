/**
 * Configuration multi-sport pour Card Metrics.
 *
...
export const SPORT_CONFIGS = {
  NHL: {
    ...
    // Mois (0-indexés) sans matchs joués : hors-saison. Sert UNIQUEMENT au
    // framing de l'UI (labels « momentum », badges « live ») — jamais au score.
    // Juillet→septembre : après la Coupe (mi-juin) et avant le camp (début oct).
...
/**
 * Vrai s'il n'y a pas de matchs en ce moment (hors-saison). À utiliser
 * UNIQUEMENT pour le framing de l'UI (labels, badges « live ») — n'affecte
 * jamais le calcul du score.
 * @param {string} [sport]
 * @param {Date} [now]
 * @returns {boolean}
 */
export function isOffseason(sport = "NHL", now = new Date()) {
  const config = getSportConfig(sport);
  const months = config.offseasonMonths ?? [];
