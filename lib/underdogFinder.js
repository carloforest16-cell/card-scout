...
const UNDERDOG_CACHE_TTL_MS = 6 * 60 * 60 * 1000;
...
/**
 * Jeunes joueurs prometteurs (cache mémoire 6 h).
 ...
 */
export async function getUnderdogPlayers() {
  const now = Date.now();
  if (
    underdogMemoryCache.payload &&
    now - underdogMemoryCache.fetchedAt < UNDERDOG_CACHE_TTL_MS
  ) {
    return structuredClone(underdogMemoryCache.payload);
  }

  const payload = await buildUnderdogPlayersFresh();
  underdogMemoryCache = { payload, fetchedAt: now };
